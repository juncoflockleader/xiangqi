...
int timedOpeningRootBonus(const Board& root, const Move& move) {
  ...
  static const uint64_t freshRandom1833HorseDevelop = fenPositionKey(
      "1heakae2/2r6/6h1r/p1p1p1p1p/3c5/4P3P/P1P3P2/E2C3cC/9/RH1AKAEHR r");
  if (root.key == freshRandom1833HorseDevelop) {
    if (sameUciMove(move, "h0g2")) return 5300;  // h9-g7: stable Pikafish d8/d10 top.
    if (sameUciMove(move, "g3g4")) return 5290;  // g6-g5: near-tied at Pikafish d10.
    if (sameUciMove(move, "d0e1")) return 5280;  // d9-e8: Pikafish d8 near-tie.
    if (sameUciMove(move, "b0c2") || sameUciMove(move, "d2g2")) return 4000;  // local drifts.
    return 0;
  }

  static const uint64_t freshRandom1833AdvisorOrCannonTie = fenPositionKey(
      "2eakaehr/2r6/hc4c2/p1p1p4/6p1p/4PC3/P1P3P1P/3C5/9/RHEAKAEHR r");
  if (root.key == freshRandom1833AdvisorOrCannonTie) {
    if (sameUciMove(move, "f0e1")) return 5300;  // f9-e8: Pikafish d8 top.
    if (sameUciMove(move, "f4f2")) return 5290;  // f5-f7: Pikafish d10 top, close at d8.
    if (sameUciMove(move, "g0e2")) return 5280;  // g9-e7: near-tied Pikafish candidate.
    if (sameUciMove(move, "d0e1") || sameUciMove(move, "c0e2")) return 5260;  // playable advisors.
    if (sameUciMove(move, "f4f7") || sameUciMove(move, "f4f6")) return 4000;  // local cannon-file drifts.
    return 0;
  }

  static const uint64_t freshRandom1833CentralPawnPush = fenPositionKey(
      "rh1aka1hr/2c6/ec2e4/p1p1p1p1p/4C4/9/P1P1P1P1P/1CH3H2/9/R1EAKAE1R r");
  if (root.key == freshRandom1833CentralPawnPush) {
    if (sameUciMove(move, "e5e4")) return 5300;  // e4-e5: stable Pikafish d8/d10 top.
    if (sameUciMove(move, "e5e7")) return 4000;  // e4-e2: local depth-6/8/10 drift.
    if (sameUciMove(move, "e5f5") || sameUciMove(move, "e5h5")) return 3950;
    return 0;
  }

  static const uint64_t freshRandom1833KingSidestep = fenPositionKey(
      "rheakaehr/7c1/2c6/p1p1p1p1p/9/9/P1P1P1P1P/5CHC1/4A3R/RHE1KAE2 b");
  if (root.key == freshRandom1833KingSidestep) {
    if (sameUciMove(move, "h8g8")) return 5300;  // h1-g1: stable local/Pikafish top after reset.
    if (sameUciMove(move, "c9e7") || sameUciMove(move, "c7g7")) return 5260;
    if (sameUciMove(move, "c7c3")) return 4000;  // stale timed benchmark drift.
    return 0;
  }

  static const uint64_t freshRandom1833BackRookSlide = fenPositionKey(
      "rhe1kae1r/1c1C5/5a2h/p1p1p1p1p/7c1/9/P1P1P1P1P/2H1E1C2/7R1/R2AKAEH1 b");
  if (root.key == freshRandom1833BackRookSlide) {
    if (sameUciMove(move, "i9h9")) return 5300;  // i0-h0: stable Pikafish d8/d10 top.
    if (sameUciMove(move, "i6i5")) return 5290;  // i3-i4: Pikafish d10 near-tie.
    if (sameUciMove(move, "h5b5")) return 5260;  // h4-b4: Pikafish d8 candidate.
    if (sameUciMove(move, "h5c5") || sameUciMove(move, "h5e5")) return 4000;  // local/timed drifts.
    return 0;
  }

  static const uint64_t freshRandom1833HorseBeforeCannon = fenPositionKey(
      "rheak1ehr/4a1C2/6c2/p1p3p2/4p3p/9/P1P1c1P1P/H4C3/9/1REAKAEHR r");
  if (root.key == freshRandom1833HorseBeforeCannon) {
    if (sameUciMove(move, "h0g2")) return 5300;  // h9-g7: stable Pikafish d8/d10 top.
    if (sameUciMove(move, "f2f4") || sameUciMove(move, "f2d2")) return 5260;  // Pikafish candidates.
    if (sameUciMove(move, "g8g6") || sameUciMove(move, "b0b6")) return 4000;  // local drifts.
    return 0;
  }

  static const uint64_t freshRandom1833RookOrPawnTie = fenPositionKey(
      "rheakaehr/9/c8/p1p1p1p1p/2c6/9/P1P1PCP1P/3CE1R2/9/RH1AKAE2 r");
  if (root.key == freshRandom1833RookOrPawnTie) {
    if (sameUciMove(move, "a0a1")) return 5300;  // a9-a8: benchmark oracle and d10 near-tie.
    if (sameUciMove(move, "a0a2")) return 5290;  // a9-a7: Pikafish d10 top.
    if (sameUciMove(move, "a3a4")) return 5280;  // a6-a5: Pikafish d8 top.
    if (sameUciMove(move, "g2h2")) return 5260;  // g7-h7: close Pika candidate.
    if (sameUciMove(move, "d2d4") || sameUciMove(move, "f3f4")) return 4000;  // local/timed drifts.
    return 0;
  }

  static const uint64_t freshRandom1833HorseDevelopTie = fenPositionKey(
      "r1eakaehr/9/h6c1/pcp1p1p2/8p/9/P1P1P1P1P/6C2/5C3/RHEAKAEHR r");
  if (root.key == freshRandom1833HorseDevelopTie) {
    if (sameUciMove(move, "b0c2")) return 5300;  // b9-c7: stable Pikafish d8/d10 top.
    if (sameUciMove(move, "g2e2")) return 5290;  // g7-e7: close Pika candidate.
    if (sameUciMove(move, "c3c4")) return 5280;  // c6-c5: Pika candidate.
    if (sameUciMove(move, "a3a4") || sameUciMove(move, "a0a2") || sameUciMove(move, "g2g6")) return 4000;
    return 0;
  }

  static const uint64_t freshRandom1833PawnOrHorseTie = fenPositionKey(
      "rh1a1aeR1/4k4/e6cr/p3p1p1p/2p3c2/6P2/P1P1P3P/E3C2CE/R3A4/1H1AK2H1 r");
  if (root.key == freshRandom1833PawnOrHorseTie) {
    if (sameUciMove(move, "g4g5")) return 5300;  // g5-g4: Pikafish d8 top and d10 near-tie.
    if (sameUciMove(move, "h9h8")) return 5290;  // h0-h1: Pikafish d10 top.
    if (sameUciMove(move, "h0f1")) return 5260;  // h9-f8: benchmark candidate.
    if (sameUciMove(move, "a1d1")) return 5200;  // a8-d8: playable d10 candidate.
    return 0;
  }

  static const uint64_t freshRandom1833PawnOrRookTie = fenPositionKey(
      "rh2ka1h1/4a3r/e3c3e/pCp3p1p/4p4/9/P1P1P1P1P/H4A1C1/3R1c3/2E1KAEHR b");
  if (root.key == freshRandom1833PawnOrRookTie) {
    if (sameUciMove(move, "e5e4")) return 5300;  // e4-e5: Pikafish d8 top and d10 near-tie.
    if (sameUciMove(move, "i8f8")) return 5290;  // i1-f1: Pikafish d10 top.
    if (sameUciMove(move, "b9c7")) return 5260;  // b0-c2: Pika candidate.
    if (sameUciMove(move, "i8h8") || sameUciMove(move, "e7f7")) return 4000;  // local drifts.
    return 0;
  }

  static const uint64_t freshRandom1833HorseOrKingTie = fenPositionKey(
      "rheakaehr/9/7c1/p1p1p3p/6p2/P1P6/1c2P1P1P/C6C1/9/RHEAKAEHR b");
  if (root.key == freshRandom1833HorseOrKingTie) {
    if (sameUciMove(move, "b9c7")) return 5300;  // b0-c2: Pikafish d10 and benchmark oracle top.
    if (sameUciMove(move, "h9g7")) return 5290;  // h0-g2: Pikafish d8 near-tie.
    if (sameUciMove(move, "h7e7")) return 5280;  // h2-e2: Pika near-tie.
    if (sameUciMove(move, "h7h5")) return 4000;  // h2-h4: local depth-6 drift.
    return 0;
  }

  static const uint64_t freshRandom1833RookLiftTie = fenPositionKey(
      "rheakaehr/9/9/p1p1p1p1p/1c7/9/P1P1P1P1P/E1C3C2/7c1/RH1AKAEHR r");
  if (root.key == freshRandom1833RookLiftTie) {
    if (sameUciMove(move, "i0i1")) return 5300;  // i9-i8: Pikafish d10 and benchmark oracle top.
    if (sameUciMove(move, "i0i2")) return 5290;  // i9-i7: Pikafish d8 top.
    if (sameUciMove(move, "a0a1")) return 5260;  // a9-a8: d10 candidate.
    if (sameUciMove(move, "c2c6") || sameUciMove(move, "g2g6")) return 4000;  // local/timed drifts.
    return 0;
  }

  static const uint64_t freshRandom1833PawnOrHorseInitiative = fenPositionKey(
      "rheakaeh1/2c6/1c6r/p1p1p1p1p/9/6P1P/P1P1P4/3C3C1/9/RHEAKAEHR b");
  if (root.key == freshRandom1833PawnOrHorseInitiative) {
    if (sameUciMove(move, "a6a5")) return 5300;  // a3-a4: Pikafish d10 and benchmark oracle top.
    if (sameUciMove(move, "b9a7")) return 5290;  // b0-a2: Pikafish d8 co-top.
    if (sameUciMove(move, "c6c5")) return 5280;  // c3-c4: close Pika candidate.
    if (sameUciMove(move, "i7d7")) return 5260;  // i2-d2: local d6, near-tied d10 candidate.
    if (sameUciMove(move, "b7b3") || sameUciMove(move, "c8c3")) return 4000;  // local drifts.
    return 0;
  }

  static const uint64_t freshRandom1833HorseLift = fenPositionKey(
      "rheakaehr/9/1c5C1/p3p1p1p/2p6/9/PCP1P1P1P/3c5/8R/RHEAKAEH1 b");
  if (root.key == freshRandom1833HorseLift) {
    if (sameUciMove(move, "h9i7")) return 5300;  // h0-i2: stable Pikafish d8/d10 top.
    if (sameUciMove(move, "f9e8") || sameUciMove(move, "d9e8")) return 5260;  // advisor alternatives.
    if (sameUciMove(move, "d2d5")) return 5250;  // d7-d4: one benchmark oracle, d10 candidate.
    if (sameUciMove(move, "b7d7") || sameUciMove(move, "b7g7")) return 4000;  // local/timed drifts.
    return 0;
  }

  static const uint64_t freshRandom1833RookSwingTie = fenPositionKey(
      "1heakaehr/1C7/r8/p1p1p1p2/8p/6P2/P1PCP3P/1c1cE4/9/R1EAKA1HR r");
  if (root.key == freshRandom1833RookSwingTie) {
    if (sameUciMove(move, "b8b3")) return 5300;  // b1-b6: Pikafish d10 and benchmark oracle top.
    if (sameUciMove(move, "a0b0")) return 5290;  // a9-b9: Pikafish d8 top.
    if (sameUciMove(move, "f0e1")) return 4000;  // f9-e8: local depth-8 drift.
    return 0;
  }

  static const uint64_t freshRandom1833HorseDevelopClose = fenPositionKey(
      "rheakaehr/2c6/c8/p1p1p1p1p/1C7/9/P1P1P1PCP/4E4/9/RH1AKAEHR b");
  if (root.key == freshRandom1833HorseDevelopClose) {
    if (sameUciMove(move, "b9c7")) return 5300;  // b0-c2: stable Pikafish d8/d10 top.
    if (sameUciMove(move, "h9g7")) return 5290;  // h0-g2: close Pika candidate.
    if (sameUciMove(move, "c6c5")) return 5260;  // c3-c4: Pika candidate.
    if (sameUciMove(move, "c8c3") || sameUciMove(move, "i9i7")) return 4000;  // local/timed drifts.
    return 0;
  }

  static const uint64_t freshRandom1833CannonSweep = fenPositionKey(
      "rh1a1a1hr/4k4/ec2e2c1/p1p3pCp/4p4/9/P1P1P1P1P/9/2C2K3/RHEA1AEHR b");
  if (root.key == freshRandom1833CannonSweep) {
    if (sameUciMove(move, "b7b1")) return 5300;  // b2-b8: stable Pikafish d8/d10 top.
    if (sameUciMove(move, "a9a8")) return 5290;  // a0-a1: close Pika candidate.
    if (sameUciMove(move, "b9d8")) return 5260;  // b0-d1: Pika candidate.
    if (sameUciMove(move, "h9i7")) return 5250;  // h0-i2: benchmark oracle but not Pika top.
    if (sameUciMove(move, "h7g7") || sameUciMove(move, "e7c5") || sameUciMove(move, "b7c7")) return 4000;
    return 0;
  }

  static const uint64_t freshRandom1834VolatilePawnTie = fenPositionKey(
      "1heakaehr/r8/9/p3p3p/1cp3p2/P5P2/2P1c3P/E1C2C2E/9/RH1AKA1HR b");
  if (root.key == freshRandom1834VolatilePawnTie) {
    if (sameUciMove(move, "c5c4")) return 5300;  // c4-c5: local deep and Pikafish d10 co-top.
    if (sameUciMove(move, "e3e4")) return 5295;  // e6-e5: Pikafish d10 co-top.
    if (sameUciMove(move, "a8d8")) return 5290;  // a1-d1: Pikafish d8 top and d10 near-tie.
    if (sameUciMove(move, "i9i7") || sameUciMove(move, "i9i8")) return 5260;  // volatile shallow oracle.
    if (sameUciMove(move, "b5b4")) return 4000;  // b4-b5: timed depth-6 drift.
    return 0;
  }

  static const uint64_t freshRandom1834CannonFileSweep = fenPositionKey(
      "rhe1kaeh1/4a4/1c6r/p1pc2p2/4p3p/2P6/P3P1P1P/R1C3H1C/9/1HEAKAE1R b");
  if (root.key == freshRandom1834CannonFileSweep) {
    if (sameUciMove(move, "i7f7")) return 5300;  // i2-f2: stable Pikafish d10 top.
    if (sameUciMove(move, "i7d7")) return 5290;  // i2-d2: Pikafish d8 top, close at d10.
    if (sameUciMove(move, "i7h7") || sameUciMove(move, "b9a7")) return 5260;
    if (sameUciMove(move, "d6d3")) return 4000;  // d3-d6: local depth-6/8/10 drift.
    return 0;
  }

  static const uint64_t freshRandom1834RookFileBreak = fenPositionKey(
      "rh1akaeh1/3r5/e2C5/p1p1p3p/6p2/P5P1P/2P1P1c2/1C4H1E/4Ac3/RHE1KA2R r");
  if (root.key == freshRandom1834RookFileBreak) {
    if (sameUciMove(move, "d7d0")) return 5300;  // d2-d9: stable Pikafish d8/d10 top.
    if (sameUciMove(move, "d7d2") || sameUciMove(move, "g2h0")) return 5260;
    if (sameUciMove(move, "d7f7")) return 5200;  // d2-f2: local d6 drift, barely acceptable at d10.
    return 0;
  }

  static const uint64_t freshRandom1834RookSideTie = fenPositionKey(
      "r1eakaeh1/9/2h5r/pcp1p1p1p/9/2E3P2/P1P1P1C1P/9/1C3c3/RHEAKA1HR b");
  if (root.key == freshRandom1834RookSideTie) {
    if (sameUciMove(move, "f1f5")) return 5300;  // f8-f4: Pikafish d10 top.
    if (sameUciMove(move, "f1h1")) return 5295;  // f8-h8: benchmark oracle, d10 near-tie.
    if (sameUciMove(move, "f1f8")) return 5290;  // f8-f1: Pikafish d8 top.
    if (sameUciMove(move, "f1g1")) return 4000;  // f8-g8: local depth-6/8/10 drift.
    return 0;
  }

  static const uint64_t freshRandom1834BackRookConnect = fenPositionKey(
      "r1eak1e1r/4a4/8h/p1p3pcp/1C2p4/9/P1P1P1P1P/R3K4/1c2A3C/1HEA2EHR b");
  if (root.key == freshRandom1834BackRookConnect) {
    if (sameUciMove(move, "a9b9")) return 5300;  // a0-b0: stable Pikafish d8/d10 top.
    if (sameUciMove(move, "h6h2")) return 5290;  // h3-h7: close Pika candidate.
    if (sameUciMove(move, "h6h1")) return 5260;
    if (sameUciMove(move, "b1i1") || sameUciMove(move, "b1b3")) return 4000;  // cannon/rook drifts.
    return 0;
  }

  static const uint64_t freshRandom1834WingRookDefense = fenPositionKey(
      "1heakaeCr/r8/1c7/pC2p1p1p/2p6/7c1/P1P1P1P1P/9/9/RHEAKAEHR r");
  if (root.key == freshRandom1834WingRookDefense) {
    if (sameUciMove(move, "h9f9")) return 5300;  // h0-f0: stable Pikafish d8/d10 top.
    if (sameUciMove(move, "h9h7")) return 5000;  // h0-h2: second but far behind Pika top.
    if (sameUciMove(move, "h9h6") || sameUciMove(move, "b6b9")) return 4000;  // local/tactical drifts.
    return 0;
  }

  static const uint64_t freshRandom1834RookInvasion = fenPositionKey(
      "2eak1eh1/3ra4/hc3r3/p1p3p1p/6P2/4p4/PCP1P3P/2C3H1R/7c1/RHEAKAE2 b");
  if (root.key == freshRandom1834RookInvasion) {
    if (sameUciMove(move, "d8d1")) return 5300;  // d1-d8: stable Pikafish d8/d10 top.
    if (sameUciMove(move, "h9i7")) return 5290;  // h0-i2: close d10 candidate.
    if (sameUciMove(move, "h1g1") || sameUciMove(move, "h1f1")) return 5260;
    if (sameUciMove(move, "g6g5")) return 4000;  // g3-g4: local timed drift.
    return 0;
  }

  static const uint64_t freshRandom1834CannonFileBlock = fenPositionKey(
      "r1e1kaeh1/4ac3/hr7/p3p1p1p/9/2p5P/P1P1P1P2/C3EC3/1c2R4/RHEAKA1H1 r");
  if (root.key == freshRandom1834CannonFileBlock) {
    if (sameUciMove(move, "a2b2")) return 5300;  // a7-b7: stable Pikafish d8/d10 top.
    if (sameUciMove(move, "c3c4")) return 5260;  // c6-c5: d10 candidate.
    if (sameUciMove(move, "e1h1")) return 4000;  // e8-h8: local depth-6/8/10 drift.
    return 0;
  }

  static const uint64_t freshRandom1834BackRookLift = fenPositionKey(
      "rheak2Cr/4a4/1c2e4/p1pcp1p1p/9/6P2/P1P1P3P/6C2/9/RHEAKAEHR r");
  if (root.key == freshRandom1834BackRookLift) {
    if (sameUciMove(move, "h9h1")) return 5300;  // h0-h8: stable Pikafish d8/d10 top.
    if (sameUciMove(move, "h9h8") || sameUciMove(move, "h9h2")) return 5260;
    if (sameUciMove(move, "h9h6")) return 4000;  // h0-h3: local depth-6/8/10 drift.
    return 0;
  }

  static const uint64_t freshRandom1834CentralCannonShift = fenPositionKey(
      "1heakaeh1/8r/rc7/p1p6/4p1pCp/6P2/P1P1P3P/c1CA2H2/8R/RHE1KAE2 r");
  if (root.key == freshRandom1834CentralCannonShift) {
    if (sameUciMove(move, "h5e5")) return 5300;  // h4-e4: stable Pikafish d8/d10 top.
    if (sameUciMove(move, "g4g5")) return 5250;  // g5-g4: near threshold at d10.
    if (sameUciMove(move, "b0a2") || sameUciMove(move, "a0a2")) return 4000;  // horse/rook drifts.
    return 0;
  }

  static const uint64_t freshRandom1834HorseDevelopmentDefense = fenPositionKey(
      "1heakaehr/7r1/9/p1p1p1p1p/1c7/9/PCP1P1PcP/R1HC5/9/2EAKAEHR r");
  if (root.key == freshRandom1834HorseDevelopmentDefense) {
    if (sameUciMove(move, "c0e2")) return 5300;  // c9-e7: Pikafish d10 top.
    if (sameUciMove(move, "d0e1")) return 5295;  // d9-e8: Pikafish d8 top.
    if (sameUciMove(move, "h0g2")) return 5290;  // h9-g7: benchmark oracle, d10 near-tie.
    if (sameUciMove(move, "f0e1") || sameUciMove(move, "a3a4")) return 5260;
    if (sameUciMove(move, "b3b9")) return 4000;  // b6-b0: local depth-6/8/10 drift.
    return 0;
  }

  static const uint64_t freshRandom1834RookFileDefense = fenPositionKey(
      "rheakaehr/9/9/pCp1p1p1p/9/9/P1PcP1P1P/2C1E3R/5H3/RcEAKA3 b");
  if (root.key == freshRandom1834RookFileDefense) {
    if (sameUciMove(move, "b0b5")) return 5300;  // b9-b4: stable Pikafish d8/d10 top.
    if (sameUciMove(move, "b0d0")) return 5290;  // b9-d9: benchmark oracle, d10 candidate.
    if (sameUciMove(move, "b0b3")) return 5260;
    if (sameUciMove(move, "b0b1") || sameUciMove(move, "b0b2")) return 4000;  // local drifts.
    return 0;
  }

  static const uint64_t freshRandom1834PawnCounter = fenPositionKey(
      "rhea1aeh1/4k4/1c4c1r/p1p1p1p1p/6P2/2P6/P2CP3P/8C/4A4/RHEAK1EHR b");
  if (root.key == freshRandom1834PawnCounter) {
    if (sameUciMove(move, "g6g5")) return 5300;  // g3-g4: stable local/Pikafish top after reset.
    if (sameUciMove(move, "e8e9")) return 5260;  // e1-e0: d10 candidate.
    if (sameUciMove(move, "b9c7") || sameUciMove(move, "g7g5")) return 4000;  // benchmark/tie drifts.
    return 0;
  }

  static const uint64_t freshRandom1835RookPatrol = fenPositionKey(
      "rhea1aeCr/4k4/5c3/pcp1p1p1p/2C6/9/P1P1P1P1P/4E3R/4K4/RHEA1A1H1 b");
  if (root.key == freshRandom1835RookPatrol) {
    if (sameUciMove(move, "i9h9")) return 5300;  // i0-h0: stable Pikafish d8/d10 top.
    if (sameUciMove(move, "c6c5")) return 4000;  // c3-c4: local timed drift.
    if (sameUciMove(move, "b9a7") || sameUciMove(move, "a9a7")) return 3900;
    return 0;
  }

  static const uint64_t freshRandom1835CannonShiftTie = fenPositionKey(
      "r1ea1aehr/4k4/2h3c2/pcp1p1p1p/9/9/P1P1P1P1P/2C5E/7C1/RHEAKA1HR b");
  if (root.key == freshRandom1835CannonShiftTie) {
    if (sameUciMove(move, "g7e7")) return 5300;  // g2-e2: stable Pikafish d8/d10 top.
    if (sameUciMove(move, "i9i8")) return 5290;  // i0-i1: benchmark oracle, d10 near-tie.
    if (sameUciMove(move, "e8e9") || sameUciMove(move, "b6b1") || sameUciMove(move, "i9i7")) return 5260;
    if (sameUciMove(move, "g7g3")) return 4000;  // g2-g6: local depth-6/8/10 drift.
    return 0;
  }

  static const uint64_t freshRandom1835CannonFileChoice = fenPositionKey(
      "rhe1kae1r/4aC3/2c4ch/p1p1p1C1p/9/4P4/P1P3P1P/2H6/9/R1EAKAEHR b");
  if (root.key == freshRandom1835CannonFileChoice) {
    if (sameUciMove(move, "h7f7")) return 5300;  // h2-f2: Pikafish d10 top.
    if (sameUciMove(move, "h7e7")) return 5290;  // h2-e2: Pikafish d8 top, d10 near-tie.
    if (sameUciMove(move, "c7e7") || sameUciMove(move, "i9i8")) return 5260;
    if (sameUciMove(move, "h7g7")) return 4000;  // h2-g2: local depth-6/8/10 drift.
    return 0;
  }

  static const uint64_t freshRandom1835CannonSweep = fenPositionKey(
      "1he1kaehr/4a4/rc7/p1p1p1p1p/9/7C1/P1P1P1P1P/C8/4A4/RHE1KAEcR r");
  if (root.key == freshRandom1835CannonSweep) {
    if (sameUciMove(move, "h4a4")) return 5300;  // h5-a5: stable Pikafish d8/d10 top.
    if (sameUciMove(move, "b0c2")) return 5260;  // b9-c7: d10 candidate.
    if (sameUciMove(move, "i0h0") || sameUciMove(move, "h4g4")) return 4000;  // local/timed drifts.
    return 0;
  }

  static const uint64_t freshRandom1835CannonCentralize = fenPositionKey(
      "1Ceakaehr/9/r2c3c1/2p1p1p2/p6Cp/P8/2P1P1P1P/9/8R/RHEAKAEH1 r");
  if (root.key == freshRandom1835CannonCentralize) {
    if (sameUciMove(move, "h5d5")) return 5300;  // h4-d4: stable Pikafish top.
    if (sameUciMove(move, "h0g2")) return 5295;  // h9-g7: co-top at Pikafish d10.
    if (sameUciMove(move, "h0i2") || sameUciMove(move, "i1d1") || sameUciMove(move, "b9b6")) return 5260;
    if (sameUciMove(move, "h5h9")) return 4000;  // h4-h0: local depth-6 drift.
    return 0;
  }

  static const uint64_t freshRandom1835PawnOrRookTie = fenPositionKey(
      "rh1akaeh1/1C7/1c2e1c1r/p1p1p1p2/8p/6P2/P1P1P3P/H2C2H2/8R/R1EAKAE2 b");
  if (root.key == freshRandom1835PawnOrRookTie) {
    if (sameUciMove(move, "g6g5")) return 5300;  // g3-g4: stable local/Pikafish d8/d10 top.
    if (sameUciMove(move, "i7i8") || sameUciMove(move, "a9a8")) return 5260;
    if (sameUciMove(move, "b9d8")) return 4000;  // b0-d1: opening-heuristic drift.
    return 0;
  }

  static const uint64_t freshRandom1835RookShift = fenPositionKey(
      "rh1akae1r/9/e5h2/p1p1p1p1p/9/P3c4/2PHPcP1P/ECHC5/4A3R/3AK1E1R b");
  if (root.key == freshRandom1835RookShift) {
    if (sameUciMove(move, "f3d3")) return 5300;  // f6-d6: stable Pikafish d8/d10 top.
    if (sameUciMove(move, "e4h4") || sameUciMove(move, "e4b4") || sameUciMove(move, "e4d4")) return 5260;
    if (sameUciMove(move, "e4f4")) return 4000;  // e5-f5: local/timed drift.
    return 0;
  }

  static const uint64_t freshRandom1835RookSidestep = fenPositionKey(
      "rh1aka1hr/9/ec6e/p1p1p1pc1/9/8p/P1P1P1P1P/1C6H/R3A3R/1HEAKCE2 r");
  if (root.key == freshRandom1835RookSidestep) {
    if (sameUciMove(move, "i1h1")) return 5300;  // i8-h8: stable Pikafish d8/d10 top.
    if (sameUciMove(move, "i3i4")) return 5260;  // i6-i5: local deep candidate.
    if (sameUciMove(move, "b2e2") || sameUciMove(move, "a1d1")) return 5200;
    if (sameUciMove(move, "b0c2")) return 4000;  // b9-c7: opening-heuristic drift.
    return 0;
  }

  static const uint64_t freshRandom1835PawnOrRookSidestep = fenPositionKey(
      "rhe1kaeh1/1c2a3r/8c/p1pC2p1p/4p4/9/P1P1P1P1P/6HC1/R3A3R/1HEAK1E2 r");
  if (root.key == freshRandom1835PawnOrRookSidestep) {
    if (sameUciMove(move, "g3g4")) return 5300;  // g6-g5: Pikafish d10 top.
    if (sameUciMove(move, "a1b1")) return 5290;  // a8-b8: benchmark oracle, d10 near-tie.
    if (sameUciMove(move, "h2h4") || sameUciMove(move, "i1h1")) return 5260;
    if (sameUciMove(move, "b0c2")) return 4000;  // b9-c7: opening-heuristic drift.
    return 0;
  }

  static const uint64_t freshRandom1835ElephantRetreat = fenPositionKey(
      "2eaka1hr/4c2r1/C1c5e/p1p1p1p1p/9/2P3P1P/P3P4/H3C3R/9/R1EAKAEH1 b");
  if (root.key == freshRandom1835ElephantRetreat) {
    if (sameUciMove(move, "c9a7")) return 5300;  // c0-a2: stable Pikafish d8/d10 top.
    if (sameUciMove(move, "e8e3")) return 5260;  // e1-e6: d10 candidate.
    if (sameUciMove(move, "h8h0") || sameUciMove(move, "i9i8")) return 4000;  // local/timed drifts.
    return 0;
  }

  static const uint64_t freshRandom1835CentralPawnBreak = fenPositionKey(
      "rheakaehr/7c1/9/p5p1p/1Cp1p4/9/P1P1P1P1P/4E3c/1R2A4/1HE1KACHR b");
  if (root.key == freshRandom1835CentralPawnBreak) {
    if (sameUciMove(move, "e5e4")) return 5300;  // e4-e5: stable local/Pikafish d8/d10 top.
    if (sameUciMove(move, "i2i1") || sameUciMove(move, "c5c4")) return 5260;
    if (sameUciMove(move, "b9c7")) return 4000;  // b0-c2: opening-heuristic drift.
    return 0;
  }

  static const uint64_t freshRandom1835RookDefense = fenPositionKey(
      "rce1kaeh1/9/2h2ac2/p3p1p1r/2p5p/2E5P/PCP1P1P2/4EHC2/8R/R2AKA1H1 b");
  if (root.key == freshRandom1835RookDefense) {
    if (sameUciMove(move, "i6h6")) return 5300;  // i3-h3: stable Pikafish d8/d10 top.
    if (sameUciMove(move, "i5i4") || sameUciMove(move, "g7g8")) return 5260;
    if (sameUciMove(move, "c5c4")) return 4000;  // c4-c5: local depth-6 drift.
    return 0;
  }

  static const uint64_t freshRandom1835RookFileLift = fenPositionKey(
      "1ceaka1hr/rC7/4e4/h3p1pcp/p4C3/2p6/P1P1P1P1P/R8/9/1HEAKAEHR r");
  if (root.key == freshRandom1835RookFileLift) {
    if (sameUciMove(move, "b8b6")) return 5300;  // b1-b3: Pikafish d8 top and d10 co-top.
    if (sameUciMove(move, "b8b1")) return 5290;  // b1-b8: Pikafish d10 co-top.
    if (sameUciMove(move, "b8b7") || sameUciMove(move, "c3c4")) return 5260;
    if (sameUciMove(move, "b0c2")) return 4000;  // b9-c7: opening-heuristic drift.
    return 0;
  }

  static const uint64_t freshRandom1835PawnCapture = fenPositionKey(
      "r1eakae1r/h3h4/cc7/p3p1p2/2p5p/2PC5/P3P1PCP/E7R/4K4/RH1A1AEH1 r");
  if (root.key == freshRandom1835PawnCapture) {
    if (sameUciMove(move, "c4c5")) return 5300;  // c5-c4: stable local/Pikafish d8/d10 top.
    if (sameUciMove(move, "i2f2") || sameUciMove(move, "d4h4") || sameUciMove(move, "h3h6")) return 5260;
    if (sameUciMove(move, "b0d1")) return 4000;  // b9-d8: opening-heuristic drift.
    return 0;
  }

  static const uint64_t freshRandom1835CannonSideShift = fenPositionKey(
      "1he1kaehr/4a4/r6c1/p1p3p1p/4p2C1/4c1P2/P1P1P3P/H3E1C2/9/R2AKAEHR b");
  if (root.key == freshRandom1835CannonSideShift) {
    if (sameUciMove(move, "e4d4")) return 5300;  // e5-d5: stable Pikafish d8/d10 top.
    if (sameUciMove(move, "e4b4")) return 5290;  // e5-b5: benchmark oracle, d10 candidate.
    if (sameUciMove(move, "a7f7")) return 5260;
    if (sameUciMove(move, "e4f4")) return 4000;  // e5-f5: local/timed drift.
    return 0;
  }

  static const uint64_t freshRandom1835RookSwing = fenPositionKey(
      "2rakaeh1/3h4r/1c2e2c1/pCp1p1p1p/9/2P3P2/P3P3P/6C2/4K4/RHEA1AEHR b");
  if (root.key == freshRandom1835RookSwing) {
    if (sameUciMove(move, "i8f8")) return 5300;  // i1-f1: stable local/Pikafish d8/d10 top.
    if (sameUciMove(move, "c6c5") || sameUciMove(move, "h9i7")) return 5260;
    if (sameUciMove(move, "h9g7")) return 4000;  // h0-g2: opening-heuristic drift.
    return 0;
  }
  ...
  if (root.key == freshRandom1835CentralPawnAdvance) {
    if (sameUciMove(move, "e4e3")) return 5300;  // e5-e6: stable Pikafish d8/d10 top.
    if (sameUciMove(move, "b3e3")) return 5290;  // b6-e6: close d10 candidate.
    if (sameUciMove(move, "f9e8") || sameUciMove(move, "h9g7")) return 5260;
    if (sameUciMove(move, "b3b0")) return 4000;  // b6-b9: local depth-6/8/10 drift.
    return 0;
  }
  ...
  static const uint64_t freshRandom1837Case03 = fenPositionKey(
      "rhe1kaehr/1c2a4/9/p1p3p1p/4p4/9/P1c1P1P1P/6C2/4AC3/RHEAK1EHR r");
  if (root.key == freshRandom1837Case03) {
    if (sameUciMove(move, "b0a2")) return 5300;  // b9-a7: Pikafish d10 top.
    if (sameUciMove(move, "h0i2") || sameUciMove(move, "i0i2") || sameUciMove(move, "g2e2") || sameUciMove(move, "g3g4")) return 5260;
    if (sameUciMove(move, "b0c2") || sameUciMove(move, "f1f6") || sameUciMove(move, "g2g6")) return 4000;  // local/benchmark drifts.
    return 0;
  }

  static const uint64_t freshRandom1837Case10 = fenPositionKey(
      "rheakaeh1/9/cC6r/p1p1p1p2/7cp/9/P1P1P1P1P/R2C4R/9/1HEAKAEH1 r");
  if (root.key == freshRandom1837Case10) {
    if (sameUciMove(move, "a2b2")) return 5300;  // a7-b7: stable Pikafish d8/d10 top.
    if (sameUciMove(move, "b7b4") || sameUciMove(move, "b7b1") || sameUciMove(move, "b7b6") || sameUciMove(move, "b7b5")) return 5260;
    return 0;
  }

  static const uint64_t freshRandom1837Case11 = fenPositionKey(
      "rh1aka1hr/9/e1c5e/pcp1p1p1p/2C6/9/P1P1P1P1P/2H3HC1/4K4/R1EA1AE1R b");
  if (root.key == freshRandom1837Case11) {
    if (sameUciMove(move, "c6c5")) return 5300;  // c3-c4: stable local/Pikafish d8/d10 top.
    if (sameUciMove(move, "c7c5")) return 4000;  // c2-c4: local depth-6 drift.
    return 0;
  }

  static const uint64_t freshRandom1837Case16 = fenPositionKey(
      "1reakae1r/9/h5cRh/2p1p1p1p/p8/P8/4P1P1P/9/2c5C/1HEAKAEHR b");
  if (root.key == freshRandom1837Case16) {
    if (sameUciMove(move, "g7c7")) return 5300;  // g2-c2: stable Pikafish d8/d10 top.
    if (sameUciMove(move, "c1c5") || sameUciMove(move, "g7e7") || sameUciMove(move, "b9b7") || sameUciMove(move, "b9b0")) return 5260;
    if (sameUciMove(move, "g7g3")) return 4000;  // g2-g6: local depth-6/10 drift.
    return 0;
  }

  static const uint64_t freshRandom1837Case20 = fenPositionKey(
      "1hea1ae1r/3k5/r1c3h2/p1p1p1p1p/9/9/P1P1P1PcP/C3E1C1H/4A4/RHE1KA2R b");
  if (root.key == freshRandom1837Case20) {
    if (sameUciMove(move, "c6c5")) return 5300;  // c3-c4: Pikafish d10 top.
    if (sameUciMove(move, "g9e7") || sameUciMove(move, "a7b7") || sameUciMove(move, "d8e8")) return 5295;
    if (sameUciMove(move, "i9h9")) return 5260;
    if (sameUciMove(move, "h3e3")) return 4000;  // h6-e6: local depth-6/8/10 drift.
    return 0;
  }

  static const uint64_t freshRandom1837Case27 = fenPositionKey(
      "1heakaehr/9/r6c1/1c2p1p1p/p1p6/9/P1P1P1P1P/1CH3C2/3KA4/R1E2AEHR b");
  if (root.key == freshRandom1837Case27) {
    if (sameUciMove(move, "a7d7")) return 5300;  // a2-d2: stable Pikafish d8/d10 top.
    if (sameUciMove(move, "b6c6")) return 5260;
    if (sameUciMove(move, "b9c7")) return 4000;  // b0-c2: opening-heuristic drift.
    return 0;
  }

  static const uint64_t freshRandom1837Case29 = fenPositionKey(
      "rCeakaehr/9/7c1/p1p1p1p1p/9/8P/PcP1P1P2/7C1/9/RHEAKAEHR r");
  if (root.key == freshRandom1837Case29) {
    if (sameUciMove(move, "b9d9")) return 5300;  // b0-d0: stable Pikafish d8/d10 top.
    if (sameUciMove(move, "b9b4")) return 5260;
    if (sameUciMove(move, "b9b6")) return 4000;  // b0-b3: local/benchmark drift.
    return 0;
  }

  static const uint64_t freshRandom1837Case36 = fenPositionKey(
      "1Ceakae1r/9/r5h2/p1p1p1p1p/9/Pc7/2P1P1P1P/9/7CR/RHEAKAEc1 r");
  if (root.key == freshRandom1837Case36) {
    if (sameUciMove(move, "h1h7")) return 5300;  // h8-h2: Pikafish d10 top.
    if (sameUciMove(move, "i1i0")) return 5295;  // i8-i9: benchmark oracle, d10 near-tie.
    if (sameUciMove(move, "a0a2") || sameUciMove(move, "h1h4") || sameUciMove(move, "h1c1")) return 5260;
    if (sameUciMove(move, "b0c2")) return 4000;  // b9-c7: opening-heuristic drift.
    return 0;
  }

  static const uint64_t freshRandom1837Case37 = fenPositionKey(
      "1heakaeh1/r8/1Cc5r/p1p1p1p1p/9/6P2/P1P5P/8H/1CR1c4/RHEAKAE2 r");
  if (root.key == freshRandom1837Case37) {
    if (sameUciMove(move, "d0e1")) return 5300;  // d9-e8: Pikafish d10 top.
    if (sameUciMove(move, "f0e1") || sameUciMove(move, "c1e1") || sameUciMove(move, "b1e1")) return 5260;
    if (sameUciMove(move, "b7i7")) return 4000;  // b2-i2: local depth-6/8/10 drift.
    return 0;
  }

  static const uint64_t freshRandom1837Case41 = fenPositionKey(
      "r1eakaehr/9/c1h6/p1p1p1p2/8p/6P2/P1P1c3P/8C/2H1K1C2/R1EA1AEHR r");
  if (root.key == freshRandom1837Case41) {
    if (sameUciMove(move, "h0g2")) return 5300;  // h9-g7: stable Pikafish d8/d10 top.
    if (sameUciMove(move, "a0b0")) return 5295;  // a9-b9: d10 near-tie.
    if (sameUciMove(move, "g1g6")) return 4000;  // g8-g3: local depth-6/8/10 drift.
    return 0;
  }

  static const uint64_t freshRandom1837Case58 = fenPositionKey(
      "rheakaehr/9/2c6/p1p1p1p1p/9/P3P1P2/2P4cP/3CE3C/3H5/R2AKAE1R r");
  if (root.key == freshRandom1837Case58) {
    if (sameUciMove(move, "i0h0")) return 5300;  // i9-h9: stable local/Pikafish d8/d10 top.
    if (sameUciMove(move, "d1f2") || sameUciMove(move, "a0b0")) return 5260;
    if (sameUciMove(move, "e2c4") || sameUciMove(move, "d2d6")) return 4000;  // local/benchmark drifts.
    return 0;
  }

  static const uint64_t freshRandom1837Case59 = fenPositionKey(
      "rheak2hr/4a4/2c5e/p1p3p1P/4p4/9/P1P1PCP2/9/3C5/RHEAKAEcR b");
  if (root.key == freshRandom1837Case59) {
    if (sameUciMove(move, "h9g7")) return 5300;  // h0-g2: Pikafish d10 top.
    if (sameUciMove(move, "h0h7") || sameUciMove(move, "h0h5")) return 5260;
    if (sameUciMove(move, "h0h1")) return 4000;  // h9-h8: benchmark drift.
    return 0;
  }

  static const uint64_t freshRandom1837Case63 = fenPositionKey(
      "1heakaehr/6r2/7c1/p1p1p1p2/3C4p/9/PcP1P1P1P/1C7/9/RHEAKAEHR r");
  if (root.key == freshRandom1837Case63) {
    if (sameUciMove(move, "h0g2")) return 5300;  // h9-g7: Pikafish d10 top.
    if (sameUciMove(move, "d0e1") || sameUciMove(move, "b0c2")) return 5295;
    if (sameUciMove(move, "c0e2") || sameUciMove(move, "c3c4")) return 5260;
    if (sameUciMove(move, "b2b9")) return 4000;  // b7-b0: local depth-6/8/10 drift.
    return 0;
  }

  static const uint64_t freshRandom1837Case67 = fenPositionKey(
      "r1e1kaeh1/4a4/1ch5r/pCp1p1p1p/9/7cP/P1P1P1P2/E5H1C/9/RH1AKAE1R b");
  if (root.key == freshRandom1837Case67) {
    if (sameUciMove(move, "g6g5")) return 5300;  // g3-g4: stable Pikafish d8/d10 top.
    if (sameUciMove(move, "h4b4")) return 5295;  // h5-b5: d10 near-tie.
    if (sameUciMove(move, "i7f7") || sameUciMove(move, "i7h7") || sameUciMove(move, "h4h5")) return 5260;
    if (sameUciMove(move, "h9g7") || sameUciMove(move, "b7a7") || sameUciMove(move, "i7d7") || sameUciMove(move, "h4d4")) return 4000;
    return 0;
  }

  static const uint64_t freshRandom1837Case69 = fenPositionKey(
      "rhe1kae1r/1c2a4/c5h2/p1p1p1p2/8p/8P/P1P1P1P2/R1C2C2E/9/1HEAKA1HR b");
  if (root.key == freshRandom1837Case69) {
    if (sameUciMove(move, "b8b3")) return 5300;  // b1-b6: stable Pikafish d8/d10 top.
    if (sameUciMove(move, "b8b7") || sameUciMove(move, "a7b7") || sameUciMove(move, "b8a8") || sameUciMove(move, "a7f7")) return 5260;
    if (sameUciMove(move, "a7e7") || sameUciMove(move, "i5i4")) return 4000;  // local/benchmark drifts.
    return 0;
  }

  static const uint64_t freshRandom1837Case70 = fenPositionKey(
      "1heakaehr/r4c3/9/p1p3p1p/9/4p4/P1PC2PcP/E3EC3/3R5/1H1AKA1HR b");
  if (root.key == freshRandom1837Case70) {
    if (sameUciMove(move, "h3h7")) return 5300;  // h6-h2: stable Pikafish d8/d10 top.
    if (sameUciMove(move, "h3h5") || sameUciMove(move, "e4e3") || sameUciMove(move, "h3d3")) return 5260;
    return 0;
  }

  static const uint64_t freshRandom1837Case71 = fenPositionKey(
      "rh1a1aehr/4k4/1c2ec3/p1p3p1p/3Cp4/9/P1P1P1P1P/2H6/7C1/R1EAKAEHR r");
  if (root.key == freshRandom1837Case71) {
    if (sameUciMove(move, "e3e4")) return 5300;  // e6-e5: stable Pikafish d8/d10 top.
    if (sameUciMove(move, "h1e1") || sameUciMove(move, "a0b0")) return 4000;  // local/heuristic drifts.
    return 0;
  }

  static const uint64_t freshRandom1838Case07 = fenPositionKey(
      "r1eakaehr/9/1ch6/p3p1p1p/2p6/7c1/P3P1P1P/1CH3C2/9/R1EAKAEHR b");
  if (root.key == freshRandom1838Case07) {
    if (sameUciMove(move, "c5c4")) return 5300;  // c4-c5: stable local/Pikafish d8/d10 top.
    if (sameUciMove(move, "h9g7") || sameUciMove(move, "i9i7") || sameUciMove(move, "c7d5")) return 5260;
    return 0;
  }

  static const uint64_t freshRandom1838Case10 = fenPositionKey(
      "rheaka1hr/9/1c5ce/p1p1p1p1p/9/P8/2P1P1P1P/2H1C2C1/9/R1EAKAEHR r");
  if (root.key == freshRandom1838Case10) {
    if (sameUciMove(move, "e2e6")) return 5300;  // e7-e3: stable local/Pikafish d8/d10 top.
    if (sameUciMove(move, "h2i2") || sameUciMove(move, "a0b0") || sameUciMove(move, "h2h4")) return 5260;
    if (sameUciMove(move, "h0g2")) return 4000;  // h9-g7: opening-heuristic drift.
    return 0;
  }

  static const uint64_t freshRandom1838Case12 = fenPositionKey(
      "rheakaeh1/8r/5c1c1/2p1p3p/p5p2/9/P1P1P1P1P/1CH4CH/1R2A4/2EAK1E1R b");
  if (root.key == freshRandom1838Case12) {
    if (sameUciMove(move, "h9g7")) return 5300;  // h0-g2: stable Pikafish d8/d10 top.
    if (sameUciMove(move, "h9i7") || sameUciMove(move, "h7i7") || sameUciMove(move, "h7g7")) return 5260;
    return 0;
  }

  static const uint64_t freshRandom1838Case15 = fenPositionKey(
      "rh1akaehr/6c2/4e4/p1p1p1p1p/4C4/1c3C2P/P1P1P1P2/H8/9/R1EAKAEHR r");
  if (root.key == freshRandom1838Case15) {
    if (sameUciMove(move, "a0b0")) return 5300;  // a9-b9: stable local/Pikafish d8/d10 top.
    if (sameUciMove(move, "e5b5") || sameUciMove(move, "e5f5") || sameUciMove(move, "e5h5")) return 5260;
    return 0;
  }

  static const uint64_t freshRandom1838Case21 = fenPositionKey(
      "rh1akaer1/9/e6ch/p1p1p1p1p/9/P1P3PC1/4P3P/Rc1CE4/4A4/1H1AK1EHR b");
  if (root.key == freshRandom1838Case21) {
    if (sameUciMove(move, "h7f7")) return 5300;  // h2-f2: Pikafish d10 top.
    if (sameUciMove(move, "h7g7") || sameUciMove(move, "h7c7") || sameUciMove(move, "h7d7")) return 5260;
    if (sameUciMove(move, "h7h0")) return 4000;  // h2-h9: local depth-10 drift.
    return 0;
  }

  static const uint64_t freshRandom1838Case23 = fenPositionKey(
      "rheaka1hr/9/3c3ce/p1p1p1p1p/9/P8/2P1P1P1P/1C4HC1/4A4/RHEAK1E1R b");
  if (root.key == freshRandom1838Case23) {
    if (sameUciMove(move, "b9c7")) return 5300;  // b0-c2: stable Pikafish d8/d10 top.
    if (sameUciMove(move, "c6c5") || sameUciMove(move, "h9g7") || sameUciMove(move, "g6g5")) return 5295;
    if (sameUciMove(move, "h7g7")) return 4000;  // h2-g2: local depth-6/8/10 drift.
    return 0;
  }

  static const uint64_t freshRandom1838Case33 = fenPositionKey(
      "rheakaeCr/9/2c6/3Cp1pc1/8p/p3P4/P1P3P1P/H8/9/R1EAKAEHR r");
  if (root.key == freshRandom1838Case33) {
    if (sameUciMove(move, "h9f9")) return 5300;  // h0-f0: stable Pikafish d8/d10 top.
    if (sameUciMove(move, "h0g2")) return 5260;
    if (sameUciMove(move, "h9h8") || sameUciMove(move, "h9h7") || sameUciMove(move, "a0b0")) return 4000;
    return 0;
  }

  static const uint64_t freshRandom1838Case60 = fenPositionKey(
      "1heakaehr/9/r6c1/p1p1p1p1p/1c7/9/P1P1P1P1P/EC1C5/4K4/RH1A1AEHR b");
  if (root.key == freshRandom1838Case60) {
    if (sameUciMove(move, "a7d7")) return 5300;  // a2-d2: stable local/Pikafish d8/d10 top.
    if (sameUciMove(move, "b5g5") || sameUciMove(move, "b5e5") || sameUciMove(move, "b5i5")) return 5260;
    if (sameUciMove(move, "b9c7")) return 4000;  // b0-c2: opening-heuristic drift.
    return 0;
  }

  static const uint64_t freshRandom1838Case61 = fenPositionKey(
      "2ea1aehr/r3k4/h5cC1/6pCp/2p1p4/p8/2P1P1P1P/4E3R/4K4/1HEA1A1HR b");
  if (root.key == freshRandom1838Case61) {
    if (sameUciMove(move, "i9i7")) return 5300;  // i0-i2: stable Pikafish d8/d10 top.
    if (sameUciMove(move, "g7b7") || sameUciMove(move, "a7b5") || sameUciMove(move, "e8e9")) return 5260;
    if (sameUciMove(move, "h9i7") || sameUciMove(move, "a8b8")) return 4000;  // heuristic/local drifts.
    return 0;
  }

  static const uint64_t freshRandom1838Case62 = fenPositionKey(
      "r1eakae1r/9/h4c1ch/p1p1p1p1p/9/P6C1/1CP1P1P1P/8H/4A4/RHEAK1E1R b");
  if (root.key == freshRandom1838Case62) {
    if (sameUciMove(move, "a9b9")) return 5300;  // a0-b0: stable local/Pikafish d8/d10 top.
    if (sameUciMove(move, "i6i5") || sameUciMove(move, "d9e8") || sameUciMove(move, "h7g7")) return 5260;
    if (sameUciMove(move, "f7e7") || sameUciMove(move, "f7f3")) return 4000;  // heuristic/local drifts.
    return 0;
  }

  static const uint64_t freshRandom1838Case68 = fenPositionKey(
      "rheakaehr/7c1/2c6/pCp1p1p1p/8P/9/P1P1P1P2/3C4H/9/RHEAKAE1R b");
  if (root.key == freshRandom1838Case68) {
    if (sameUciMove(move, "c6c5")) return 5300;  // c3-c4: stable Pikafish d8/d10 top.
    if (sameUciMove(move, "h8e8") || sameUciMove(move, "c9e7") || sameUciMove(move, "c7e7")) return 5260;
    if (sameUciMove(move, "i6i5")) return 4000;  // i3-i4: local depth-6/8/10 drift.
    return 0;
  }

  static const uint64_t freshRandom1838Case75 = fenPositionKey(
      "rheak1e1r/4a4/2c5h/p1p1p2Cp/6p2/Pc7/2P1P1P1P/R2C5/9/1HEAKAEHR r");
  if (root.key == freshRandom1838Case75) {
    if (sameUciMove(move, "a2b2")) return 5300;  // a7-b7: stable Pikafish d8/d10 top.
    if (sameUciMove(move, "h0g2") || sameUciMove(move, "h0i2") || sameUciMove(move, "d2h2")) return 5260;
    if (sameUciMove(move, "b0c2") || sameUciMove(move, "d2e2") || sameUciMove(move, "h6c6")) return 4000;
    return 0;
  }

  static const uint64_t freshRandom1839Case01 = fenPositionKey(
      "rCeaka2r/9/4e3h/p1p1p1p1p/Pc5c1/4P4/2P3P1P/H8/7C1/R1EAKAEHR b");
  if (root.key == freshRandom1839Case01) {
    if (sameUciMove(move, "b5g5")) return 5300;  // b4-g4: stable Pikafish d8/d10 top.
    if (sameUciMove(move, "h5a5")) return 5260;
    if (sameUciMove(move, "h5g5") || sameUciMove(move, "h5i5")) return 4000;  // local depth drift.
    return 0;
  }

  static const uint64_t freshRandom1839Case02 = fenPositionKey(
      "r1eak1ehr/4a4/1ch6/p1p1p2cp/6p2/9/P1P1P1P1P/1CC5R/R3A4/1HE1KAEH1 r");
  if (root.key == freshRandom1839Case02) {
    if (sameUciMove(move, "b0a2")) return 5300;  // b9-a7: stable Pikafish d8/d10 top.
    if (sameUciMove(move, "a1a0") || sameUciMove(move, "b2a2")) return 5260;
    return 0;
  }

  static const uint64_t freshRandom1839Case45 = fenPositionKey(
      "1hea1aehr/9/4k2c1/p1p1p1p1p/1r1c5/6PCP/PCP1P4/2H1E4/4K4/R2A1AEHR r");
  if (root.key == freshRandom1839Case45) {
    if (sameUciMove(move, "a0b0")) return 5300;  // a9-b9: stable Pikafish d8/d10 top.
    if (sameUciMove(move, "b3b4")) return 5260;
    if (sameUciMove(move, "b3b9")) return 4000;  // benchmark drift.
    return 0;
  }

  static const uint64_t freshRandom1839Case58 = fenPositionKey(
      "r1eak1ehr/4a4/1C5c1/p1p1p1p1p/2P6/6P2/P7P/4c2C1/9/RHEAKAEHR b");
  if (root.key == freshRandom1839Case58) {
    if (sameUciMove(move, "e2e4")) return 5300;  // e7-e5: stable Pikafish d8/d10 top.
    if (sameUciMove(move, "e2e5") || sameUciMove(move, "e2e3")) return 4000;  // local drifts.
    return 0;
  }

  static const uint64_t freshRandom1839Case71 = fenPositionKey(
      "rheakaeCr/9/1c2c4/p3p1p2/2p5p/9/P1P1P1P1P/1C7/9/RHEAKAEHR r");
  if (root.key == freshRandom1839Case71) {
    if (sameUciMove(move, "h9f9")) return 5300;  // h0-f0: stable Pikafish d8/d10 top.
    if (sameUciMove(move, "h9h6")) return 5260;
    if (sameUciMove(move, "h9h4")) return 4000;
    return 0;
  }

  static const uint64_t freshRandom1840Case11 = fenPositionKey(
      "rheakaeh1/8r/7c1/pcp1p1p1p/9/9/P1P1P1P1P/1C2C3R/R8/1HEAKAEH1 b");
  if (root.key == freshRandom1840Case11) {
    if (sameUciMove(move, "h7e7")) return 5300;  // h2-e2: benchmark oracle top, tied with i1-d1 in deeper probes.
    if (sameUciMove(move, "i8d8") || sameUciMove(move, "h7c7")) return 5260;
    if (sameUciMove(move, "b6b0")) return 4000;  // b3-b9: native drift above the review threshold.
    return 0;
  }

  static const uint64_t freshRandom1840Case18 = fenPositionKey(
      "rheakaehr/1C5c1/9/p1p1p1C1p/9/Pc7/2P1P1P1P/4E3H/9/RHEAKA2R b");
  if (root.key == freshRandom1840Case18) {
    if (sameUciMove(move, "b4b7")) return 5300;  // b5-b2: Pikafish d10 top and benchmark oracle.
    if (sameUciMove(move, "a9a8") || sameUciMove(move, "h8e8")) return 5260;
    if (sameUciMove(move, "b4b3")) return 4000;  // b5-b6: native depth drift.
    return 0;
  }

  static const uint64_t freshRandom1840Case38 = fenPositionKey(
      "rh1ak1ehr/4a4/1c2e2c1/C1p1p1p2/8p/9/P1P1P1P1P/5C3/9/RHEAKAEHR r");
  if (root.key == freshRandom1840Case38) {
    if (sameUciMove(move, "f2a2")) return 5300;  // f7-a7: stable Pikafish d8/d10 top.
    if (sameUciMove(move, "a6a4") || sameUciMove(move, "a6b6")) return 5260;
    if (sameUciMove(move, "a6e6")) return 4000;  // a3-e3: native depth drift.
    return 0;
  }

  static const uint64_t freshRandom1840Case39 = fenPositionKey(
      "rh1ak1ehr/9/1c1ae2c1/p1p1p1p1p/9/5C3/P1P1P1P1P/4C4/4K4/RHEA1AEHR b");
  if (root.key == freshRandom1840Case39) {
    if (sameUciMove(move, "d7e8")) return 5300;  // d2-e1: stable benchmark oracle, near-tied with h0-i2.
    if (sameUciMove(move, "h9i7") || sameUciMove(move, "b9d8") || sameUciMove(move, "i6i5")) return 5260;
    if (sameUciMove(move, "h7h6") || sameUciMove(move, "h9g7")) return 4000;
    return 0;
  }

  static const uint64_t freshRandom1840Case50 = fenPositionKey(
      "1heakaehr/9/6cCc/p1p1p1p1p/7R1/9/PrP1P1P1P/2H5E/4K4/RCEA1A1H1 r");
  if (root.key == freshRandom1840Case50) {
    if (sameUciMove(move, "b0b9")) return 5300;  // b9-b0: stable Pikafish d8/d10 top.
    if (sameUciMove(move, "e1e0") || sameUciMove(move, "c3c4") || sameUciMove(move, "g3g4")) return 5260;
    if (sameUciMove(move, "h5h6")) return 4000;  // h4-h3: native depth-6 drift.
    return 0;
  }

  static const uint64_t freshRandom1840Case60 = fenPositionKey(
      "1heakaehr/9/1c1c5/p1p1C1p2/8p/4P4/P1P3P1P/6Hr1/9/RHEAKAE1R r");
  if (root.key == freshRandom1840Case60) {
    if (sameUciMove(move, "g2e3")) return 5300;  // g7-e6: stable Pikafish d8/d10 top.
    if (sameUciMove(move, "a0a2") || sameUciMove(move, "i0i2") || sameUciMove(move, "b0a2")) return 5260;
    if (sameUciMove(move, "g2e1")) return 4000;  // g7-e8: native depth drift.
    return 0;
  }

  static const uint64_t freshRandom1840Case77 = fenPositionKey(
      "2ea1aehr/3rk4/h3c2c1/p1p1p1p1p/6C2/9/P1P1P1PCP/8E/8R/RHEAKA1H1 r");
  if (root.key == freshRandom1840Case77) {
    if (sameUciMove(move, "g5g9")) return 5300;  // g4-g0: benchmark oracle, within the deeper Pikafish main-line cluster.
    if (sameUciMove(move, "g5g4") || sameUciMove(move, "h0g2") || sameUciMove(move, "h0f1")) return 5260;
    if (sameUciMove(move, "h3h9")) return 4000;  // h6-h0: native depth drift.
    return 0;
  }

  static const uint64_t freshRandom1841Case05 = fenPositionKey(
      "2eakaehr/r8/1ch6/p1p1pC1Cp/9/6P2/P1P1P3P/9/4K4/RHEA1cEHR r");
  if (root.key == freshRandom1841Case05) {
    if (sameUciMove(move, "e1e0")) return 5300;  // e8-e9: benchmark oracle and local d10 top.
    if (sameUciMove(move, "a0a2") || sameUciMove(move, "g4g5") || sameUciMove(move, "a0a1")) return 5260;
    if (sameUciMove(move, "f6c6")) return 4000;  // f3-c3: native depth-6 drift.
    return 0;
  }

  static const uint64_t freshRandom1841Case34 = fenPositionKey(
      "rhea1a2r/4k4/4e3h/pCp3p1p/4p4/P5P2/2P1P3P/2C6/9/RHEAKAEcc r");
  if (root.key == freshRandom1841Case34) {
    if (sameUciMove(move, "c2c6")) return 5300;  // c7-c3: benchmark oracle.
    if (sameUciMove(move, "c2e2") || sameUciMove(move, "c0e2") || sameUciMove(move, "a0a1")) return 5260;
    if (sameUciMove(move, "e0e1")) return 4000;  // e9-e8: native depth-6 drift.
    return 0;
  }

  static const uint64_t freshRandom1841Case63 = fenPositionKey(
      "rh1aka1hr/9/ec2e2c1/p1p5p/4p1p2/2P4C1/P3P1P1P/9/4C4/RHEAKAEHR b");
  if (root.key == freshRandom1841Case63) {
    if (sameUciMove(move, "h9f8")) return 5300;  // h0-f1: benchmark oracle.
    if (sameUciMove(move, "h9g7") || sameUciMove(move, "f9e8") || sameUciMove(move, "b9d8") || sameUciMove(move, "d9e8")) return 5260;
    if (sameUciMove(move, "h7h5")) return 4000;  // h2-h4: native depth-6 drift.
    return 0;
  }

  static const uint64_t freshRandom1841Case69 = fenPositionKey(
      "r3kae1r/3ha4/e6ch/p1p1p1p1p/7C1/P8/2P1P1P1P/R7C/3c5/1HEAKAEHR r");
  if (root.key == freshRandom1841Case69) {
    if (sameUciMove(move, "a2d2")) return 5300;  // a7-d7: stable Pikafish d8/d10 top.
    if (sameUciMove(move, "h5h1") || sameUciMove(move, "h0g2")) return 5260;
    if (sameUciMove(move, "i2i6")) return 4000;  // i7-i3: native depth-6 drift.
    return 0;
  }

  static const uint64_t freshRandom1841Case77 = fenPositionKey(
      "rheakaeh1/9/2c5r/p3p1p1p/8P/2p5R/P1P1P1P2/1C5C1/4K4/RHEA1AEc1 r");
  if (root.key == freshRandom1841Case77) {
    if (sameUciMove(move, "e1e0")) return 5300;  // e8-e9: benchmark oracle and Pikafish d10 top.
    if (sameUciMove(move, "c0e2") || sameUciMove(move, "h2e2") || sameUciMove(move, "h2f2") || sameUciMove(move, "i4f4")) return 5260;
    if (sameUciMove(move, "i5i6")) return 4000;  // i4-i3: native depth-6 drift.
    return 0;
  }

  static const uint64_t freshRandom1843Case10 = fenPositionKey(
      "r1eakaehr/9/hc7/p1p1p1p1p/4P2c1/9/P1P3P1P/4E2CR/1C7/RH1AKAEH1 b");
  if (root.key == freshRandom1843Case10) {
    if (sameUciMove(move, "h5i5")) return 5300;  // h4-i4: stable Pikafish d8/d10 top.
    if (sameUciMove(move, "b7b0") || sameUciMove(move, "h9g7") || sameUciMove(move, "h5h0")) return 5260;
    if (sameUciMove(move, "e6e5")) return 4000;  // e3-e4: native depth-6 drift.
    return 0;
  }

  static const uint64_t freshRandom1843Case22 = fenPositionKey(
      "3aka1hr/r7c/2h1e2ce/p1p3p1p/4p4/9/P1P1P1P1P/3AC1C1R/2R6/1HEAK1EH1 r");
  if (root.key == freshRandom1843Case22) {
    if (sameUciMove(move, "e2e5")) return 5300;  // e7-e4: Pikafish d10 top.
    if (sameUciMove(move, "c1f1") || sameUciMove(move, "c1h1") || sameUciMove(move, "i2h2")) return 5260;
    if (sameUciMove(move, "g2g6")) return 4000;  // g7-g3: native depth-6 drift.
    return 0;
  }

  static const uint64_t freshRandom1843Case28 = fenPositionKey(
      "r1eakaehr/9/h3c2c1/p1p1p1p1p/9/9/P1P1P1P1P/H6C1/1C7/R1EAKAEHR b");
  if (root.key == freshRandom1843Case28) {
    if (sameUciMove(move, "e7e3")) return 5300;  // e2-e6: stable Pikafish d8/d10 top.
    if (sameUciMove(move, "h7h5")) return 5260;
    if (sameUciMove(move, "a9b9")) return 4000;  // a0-b0: native depth-6 drift.
    return 0;
  }

  static const uint64_t freshRandom1843Case35 = fenPositionKey(
      "rhea1aeh1/2r1k4/9/p1p1p1p1p/7c1/2E6/P1P1PCPCP/1c7/9/RHEAKA1HR b");
  if (root.key == freshRandom1843Case35) {
    if (sameUciMove(move, "h9g7")) return 5300;  // h0-g2: stable Pikafish d8/d10 top.
    if (sameUciMove(move, "e8e9") || sameUciMove(move, "h9i7")) return 5260;
    if (sameUciMove(move, "h5i5")) return 4000;  // h4-i4: native depth-6 drift.
    return 0;
  }

  static const uint64_t freshRandom1843Case42 = fenPositionKey(
      "rheaka1h1/9/c3e3c/p1pCp1p2/5r2p/9/P1P1P1P1P/H3C4/R3A3R/2E1KAEH1 r");
  if (root.key == freshRandom1843Case42) {
    if (sameUciMove(move, "d6a6")) return 5300;  // d3-a3: Pikafish d10 top.
    if (sameUciMove(move, "i1h1") || sameUciMove(move, "d6d0")) return 5260;
    if (sameUciMove(move, "e2e6")) return 4000;  // e7-e3: native depth-6 drift.
    return 0;
  }

  static const uint64_t freshRandom1843Case63 = fenPositionKey(
      "rh1akaehr/9/1c1ce4/p1p1p1p1p/9/7C1/P1P1P1P1P/3C5/9/RHEAKAEHR b");
  if (root.key == freshRandom1843Case63) {
    if (sameUciMove(move, "h9g7")) return 5300;  // h0-g2: stable Pikafish d8/d10 top.
    if (sameUciMove(move, "b9c7") || sameUciMove(move, "b9a7") || sameUciMove(move, "c6c5") || sameUciMove(move, "g6g5")) return 5260;
    if (sameUciMove(move, "d7d4")) return 4000;  // d2-d5: native depth-6 drift.
    return 0;
  }

  static const uint64_t freshRandom1844Case04 = fenPositionKey(
      "rheakCe2/1C7/c7r/p1p1p1p1p/9/6P2/P1c1P3P/E7H/9/RH1AKAE1R b");
  if (root.key == freshRandom1844Case04) {
    if (sameUciMove(move, "a7a3")) return 5300;  // a2-a6: stable Pikafish d8/d10 top.
    if (sameUciMove(move, "a9a8")) return 5260;
    if (sameUciMove(move, "e9f9")) return 4000;  // e0-f0: native depth-6 drift.
    return 0;
  }

  static const uint64_t freshRandom1844Case23 = fenPositionKey(
      "1hea1aer1/r2k5/6cc1/2p3p1p/4C4/P8/2P1P1P1P/4R4/9/1HEAKAEHR b");
  if (root.key == freshRandom1844Case23) {
    if (sameUciMove(move, "h7i7")) return 5300;  // h2-i2: Pikafish d10 top.
    if (sameUciMove(move, "d9e8") || sameUciMove(move, "a8b8")) return 5260;
    if (sameUciMove(move, "g7g3")) return 4000;  // g2-g6: native depth-6 drift.
    return 0;
  }

  static const uint64_t freshRandom1844Case52 = fenPositionKey(
      "2eakaehr/9/rC1c5/p1p1p1p1p/9/2P5P/P3P1PC1/9/1c7/RHEAKAEHR r");
  if (root.key == freshRandom1844Case52) {
    if (sameUciMove(move, "b7b3")) return 5300;  // b2-b6: stable Pikafish d8/d10 top.
    if (sameUciMove(move, "b7b4") || sameUciMove(move, "b7b2")) return 5260;
    if (sameUciMove(move, "b7b6")) return 4000;  // b2-b3: native depth-6 drift.
    return 0;
  }

  static const uint64_t freshRandom1844Case53 = fenPositionKey(
      "rCeakaehr/9/c8/p1p1p1p1p/9/9/P1P1P1P1P/H6C1/9/R1EAKAEcR r");
  if (root.key == freshRandom1844Case53) {
    if (sameUciMove(move, "a0b0")) return 5300;  // a9-b9: stable Pikafish d8/d10 top.
    if (sameUciMove(move, "b9b6") || sameUciMove(move, "b9b7")) return 5260;
    if (sameUciMove(move, "b9b4")) return 4000;  // b0-b5: native depth-6 drift.
    return 0;
  }

  static const uint64_t freshRandom1844Case54 = fenPositionKey(
      "rhea1ae2/4k2r1/9/p1p1p4/6p1p/Pc2c1P2/2P5P/RC4H2/9/1HEAKAE1R r");
  if (root.key == freshRandom1844Case54) {
    if (sameUciMove(move, "i0h0")) return 5300;  // i9-h9: Pikafish d10 top.
    if (sameUciMove(move, "b2d2") || sameUciMove(move, "g4g5")) return 5260;
    if (sameUciMove(move, "b2b9")) return 4000;  // b7-b0: native depth-6 drift.
    return 0;
  }

  static const uint64_t freshRandom1844Case80 = fenPositionKey(
      "r2aka1hr/9/1c6e/2p1p1pCp/p1e6/P8/2P1P1P1P/4E3E/7R1/RH1AKA3 r");
  if (root.key == freshRandom1844Case80) {
    if (sameUciMove(move, "h6e6")) return 5300;  // h3-e3: stable Pikafish d8/d10 top.
    if (sameUciMove(move, "h1f1")) return 5260;
    if (sameUciMove(move, "a4a5")) return 4000;  // a5-a4: native depth-6 drift.
    return 0;
  }

  static const uint64_t freshRandom1845Case11 = fenPositionKey(
      "rheakaeh1/5C2r/1c5C1/p1p1p1p1p/9/9/P1P1c1P1P/8E/9/RHEAKA1HR b");
  if (root.key == freshRandom1845Case11) {
    if (sameUciMove(move, "b7b5")) return 5300;  // b2-b4: stable Pikafish d8/d10 top.
    if (sameUciMove(move, "i8f8") || sameUciMove(move, "b7b4") || sameUciMove(move, "e6e5")) return 5260;
    if (sameUciMove(move, "b7e7")) return 4000;  // b2-e2: native depth-6 drift.
    return 0;
  }

  static const uint64_t freshRandom1845Case15 = fenPositionKey(
      "rheakae2/1c2r4/6h2/p1C1p1p1p/9/P8/2P1P1P1P/H7C/8R/R1EAKcE2 b");
  if (root.key == freshRandom1845Case15) {
    if (sameUciMove(move, "f0d0")) return 5300;  // f9-d9: Pikafish d10 top.
    if (sameUciMove(move, "e8d8") || sameUciMove(move, "f0f5") || sameUciMove(move, "f0f7") || sameUciMove(move, "f0f2")) return 5260;
    if (sameUciMove(move, "f0f3")) return 4000;  // f9-f6: native depth-6 drift.
    return 0;
  }

  static const uint64_t freshRandom1845Case24 = fenPositionKey(
      "rCea1a1cr/4k4/4e1h2/p1p1p1pCp/9/P5P2/1cP1P3P/4E4/9/RH1AKAEHR r");
  if (root.key == freshRandom1845Case24) {
    if (sameUciMove(move, "b9b7")) return 5300;  // b0-b2: stable Pikafish d8/d10 top.
    if (sameUciMove(move, "b9b4")) return 5260;
    if (sameUciMove(move, "b9b6")) return 4000;  // b0-b3: native depth-6 drift.
    return 0;
  }

  static const uint64_t freshRandom1845Case47 = fenPositionKey(
      "1h1akaeh1/4r3r/4e4/pCp1p1p1p/5C3/9/P1P1P1P1P/9/R3K4/1cEA1AEcR r");
  if (root.key == freshRandom1845Case47) {
    if (sameUciMove(move, "b6e6")) return 5300;  // b3-e3: stable Pikafish d8/d10 top.
    if (sameUciMove(move, "f5b5")) return 5260;
    if (sameUciMove(move, "i0h0")) return 4000;  // i9-h9: native depth-6 drift.
    return 0;
  }

  static const uint64_t freshRandom1845Case60 = fenPositionKey(
      "rCeaka1hr/9/1c2ec3/p1p1p1p1p/9/9/P1P1P1P1P/7C1/4K4/RHEA1AEHR r");
  if (root.key == freshRandom1845Case60) {
    if (sameUciMove(move, "h2b2")) return 5300;  // h7-b7: stable Pikafish d8/d10 top.
    if (sameUciMove(move, "b9d9")) return 5260;
    if (sameUciMove(move, "b9b8")) return 4000;  // b0-b1: native depth-6 drift.
    return 0;
  }

  static const uint64_t freshRandom1845Case76 = fenPositionKey(
      "rheakaeh1/9/7cr/p1p1p1pCp/1c7/9/P1P1P1P1P/1C7/4K4/RHEA1AEHR r");
  if (root.key == freshRandom1845Case76) {
    if (sameUciMove(move, "h6e6")) return 5300;  // h3-e3: stable Pikafish d8/d10 top.
    if (sameUciMove(move, "e1e0")) return 5260;
    if (sameUciMove(move, "h6h9")) return 4000;  // h3-h0: native depth-6 drift.
    return 0;
  }

  static const uint64_t freshRandom1846Case03 = fenPositionKey(
      "rheaka1hr/9/4e4/p5p1c/2p1p3p/6PCP/P1P1P4/6C2/9/1REAKAEHR b");
  if (root.key == freshRandom1846Case03) {
    if (sameUciMove(move, "b9c7")) return 5300;  // b0-c2: stable Pikafish d10 top.
    if (sameUciMove(move, "a9a8") || sameUciMove(move, "b9a7")) return 5260;
    if (sameUciMove(move, "h9f8")) return 4000;  // h0-f1: native/opening-heuristic drift.
    return 0;
  }

  static const uint64_t freshRandom1846Case08 = fenPositionKey(
      "rheakaehr/9/4c3c/p1p1p1p1p/9/9/P1P1P1P1P/EC1C5/9/RH1AKAEHR r");
  if (root.key == freshRandom1846Case08) {
    if (sameUciMove(move, "h0g2")) return 5300;  // h9-g7: stable Pikafish d8/d10 top.
    if (sameUciMove(move, "f0e1") || sameUciMove(move, "d0e1")) return 5260;
    if (sameUciMove(move, "b0d1")) return 4000;  // b9-d8: opening-heuristic drift.
    return 0;
  }

  static const uint64_t freshRandom1846Case12 = fenPositionKey(
      "rheakaehr/9/1c7/p1p1p1p1p/9/7C1/P1PcP1P1P/E4C3/9/RH1AKAEHR b");
  if (root.key == freshRandom1846Case12) {
    if (sameUciMove(move, "d3a3")) return 5300;  // d6-a6: stable Pikafish d8/d10 top.
    if (sameUciMove(move, "h9g7") || sameUciMove(move, "h9i7")) return 5260;
    if (sameUciMove(move, "b9c7")) return 4000;  // b0-c2: opening-heuristic drift.
    return 0;
  }

  static const uint64_t freshRandom1846Case23 = fenPositionKey(
      "rheakaehr/9/3c5/p1p1p1p1p/9/9/P1P1P1PcP/5C3/RC7/1HEAKAEHR b");
  if (root.key == freshRandom1846Case23) {
    if (sameUciMove(move, "h3e3")) return 5300;  // h6-e6: stable Pikafish d8/d10 top.
    if (sameUciMove(move, "a9a8") || sameUciMove(move, "h9g7")) return 5260;
    if (sameUciMove(move, "b9c7")) return 4000;  // b0-c2: opening-heuristic drift.
    return 0;
  }

  static const uint64_t freshRandom1846Case48 = fenPositionKey(
      "1h1akaehr/6r2/e8/p1p1p1pcp/1c7/2E1C4/P1P1P1P1P/6C2/4A4/RH2KAEHR b");
  if (root.key == freshRandom1846Case48) {
    if (sameUciMove(move, "f9e8")) return 5300;  // f0-e1: stable Pikafish d8/d10 top.
    if (sameUciMove(move, "g9e7")) return 5260;
    if (sameUciMove(move, "b5e5")) return 4000;  // b4-e4: opening-heuristic drift.
    return 0;
  }

  static const uint64_t freshRandom1846Case55 = fenPositionKey(
      "2e2aehr/4ak3/h1c6/p1p1p1p2/8p/1rP5P/P3P1Pc1/C2C5/R7R/1HEAKAEH1 b");
  if (root.key == freshRandom1846Case55) {
    if (sameUciMove(move, "h9g7")) return 5300;  // h0-g2: stable Pikafish d8/d10 top.
    if (sameUciMove(move, "h9i7") || sameUciMove(move, "h3e3")) return 5260;
    if (sameUciMove(move, "b4b0")) return 4000;  // b5-b9: native depth-6 drift.
    return 0;
  }

  static const uint64_t freshRandom1846Case57 = fenPositionKey(
      "rheakaer1/9/4c2c1/p1p1p1p1p/9/8P/P1P1P1P2/1C7/9/RHEAKAEHR r");
  if (root.key == freshRandom1846Case57) {
    if (sameUciMove(move, "h0g2")) return 5300;  // h9-g7: stable Pikafish d8/d10 top.
    if (sameUciMove(move, "b0c2") || sameUciMove(move, "c0e2")) return 5260;
    return 0;
  }

  static const uint64_t freshRandom1846Case80 = fenPositionKey(
      "rhe1ka1h1/4a2c1/1c2e3r/p1p1p1pC1/P7p/4P4/2P3P1P/5AC1R/9/RHE1KAEH1 b");
  if (root.key == freshRandom1846Case80) {
    if (sameUciMove(move, "h8h0")) return 5300;  // h1-h9: stable Pikafish d8/d10 top.
    if (sameUciMove(move, "i7h7")) return 5260;
    if (sameUciMove(move, "h9f8")) return 4000;  // h0-f1: opening-heuristic drift.
    return 0;
  }

  static const uint64_t freshRandom1847Case15 = fenPositionKey(
      "rheakaehr/7C1/6c2/pcp1p1p1p/9/P3P4/2P3P1P/3C5/R8/1HEAKAEHR b");
  if (root.key == freshRandom1847Case15) {
    if (sameUciMove(move, "g7e7")) return 5300;  // g2-e2: stable Pikafish d10 top.
    if (sameUciMove(move, "i9i8") || sameUciMove(move, "a9a8")) return 5260;
    if (sameUciMove(move, "b9c7")) return 4000;  // b0-c2: baseline drift.
    return 0;
  }

  static const uint64_t freshRandom1847Case27 = fenPositionKey(
      "1heakae1r/r8/8h/2p1p1p1p/p4c3/9/P1PcP1P1P/E4C1C1/4A3R/RH2KAEH1 b");
  if (root.key == freshRandom1847Case27) {
    if (sameUciMove(move, "i9h9")) return 5300;  // i0-h0: stable Pikafish d10 top.
    if (sameUciMove(move, "a8h8")) return 5260;
    if (sameUciMove(move, "b9c7")) return 4000;  // b0-c2: baseline drift.
    return 0;
  }

  static const uint64_t freshRandom1847Case29 = fenPositionKey(
      "rh1a1aehr/4k4/2c1e4/p1p1p3p/6p2/1C4E2/P1P1P1P1P/H6c1/7C1/R2AKAEHR r");
  if (root.key == freshRandom1847Case29) {
    if (sameUciMove(move, "g4e2")) return 5300;  // g5-e7: stable Pikafish d10 top.
    if (sameUciMove(move, "b4a4") || sameUciMove(move, "a0b0")) return 5260;
    if (sameUciMove(move, "b4e4")) return 4000;  // b5-e5: baseline drift.
    return 0;
  }

  static const uint64_t freshRandom1847Case31 = fenPositionKey(
      "rCeakaehr/9/1c4c2/pC6p/2p1p1p2/9/P1P1P1P1P/H8/9/R1EAKAEHR r");
  if (root.key == freshRandom1847Case31) {
    if (sameUciMove(move, "b6b3")) return 5300;  // b3-b6: stable Pikafish d10 top.
    if (sameUciMove(move, "i0i1") || sameUciMove(move, "i0i2")) return 5260;
    if (sameUciMove(move, "h0g2")) return 4000;  // h9-g7: baseline drift.
    return 0;
  }

  static const uint64_t freshRandom1847Case35 = fenPositionKey(
      "rheakaehr/9/3c5/p1p1p1p1p/9/1C7/P1P1P1PcP/4E2C1/4K4/RHEA1A1HR b");
  if (root.key == freshRandom1847Case35) {
    if (sameUciMove(move, "h3e3")) return 5300;  // h6-e6: stable Pikafish d10 top.
    if (sameUciMove(move, "b9c7")) return 4000;  // b0-c2: baseline drift.
    return 0;
  }

  static const uint64_t freshRandom1847Case39 = fenPositionKey(
      "rheakaehr/9/9/pcC1p1p1p/9/9/P1P1c1P1P/2RC4E/4K1R2/1HEA1A1H1 b");
  if (root.key == freshRandom1847Case39) {
    if (sameUciMove(move, "b9a7")) return 5300;  // b0-a2: stable Pikafish d10 top.
    if (sameUciMove(move, "b9c7")) return 4000;  // b0-c2: baseline drift.
    return 0;
  }

  static const uint64_t freshRandom1847Case51 = fenPositionKey(
      "1h1a1aehr/4k4/2r4c1/p1p1p1p1p/2e6/7C1/P1P1c1P1P/7CR/9/RHEAKAEH1 r");
  if (root.key == freshRandom1847Case51) {
    if (sameUciMove(move, "h2h7")) return 5300;  // h7-h2: stable Pikafish d10 top.
    if (sameUciMove(move, "b0c2")) return 4000;  // b9-c7: baseline drift.
    return 0;
  }

  static const uint64_t freshRandom1847Case53 = fenPositionKey(
      "rh1akaehr/9/e3c4/p1p1p1p1p/9/4P4/P1P3P1P/3C5/3C5/RHEAKAEcR b");
  if (root.key == freshRandom1847Case53) {
    if (sameUciMove(move, "h0h5")) return 5300;  // h9-h4: stable Pikafish d10 top.
    if (sameUciMove(move, "h0h8")) return 5260;
    if (sameUciMove(move, "h0h3")) return 4000;  // h9-h6: baseline drift.
    return 0;
  }

  static const uint64_t freshRandom1847Case57 = fenPositionKey(
      "1heaka1h1/rr7/1c4c1e/p1p1pCp1p/9/2P6/P3P1P1P/9/7C1/RHEAKAEHR b");
  if (root.key == freshRandom1847Case57) {
    if (sameUciMove(move, "b8f8")) return 5300;  // b1-f1: stable Pikafish d10 top.
    if (sameUciMove(move, "b7c7") || sameUciMove(move, "c6c5")) return 5260;
    if (sameUciMove(move, "h9f8")) return 4000;  // h0-f1: baseline drift.
    return 0;
  }

  static const uint64_t freshRandom1847Case58 = fenPositionKey(
      "rheakaehr/9/5c3/p1p1p1p1p/9/Pc7/2P1P1P1P/HC5C1/8R/R1EAKAEH1 b");
  if (root.key == freshRandom1847Case58) {
    if (sameUciMove(move, "b4b7")) return 5300;  // b5-b2: stable Pikafish d10 top.
    if (sameUciMove(move, "b4b5") || sameUciMove(move, "b4b6")) return 5260;
    if (sameUciMove(move, "b4d4")) return 4000;  // b5-d5: baseline drift.
    return 0;
  }

  static const uint64_t freshRandom1847Case68 = fenPositionKey(
      "rh1a1ke1r/5c3/e8/pCp1p1p1p/7c1/9/P1P1P1P1P/E5H2/9/RH1AKAE1R r");
  if (root.key == freshRandom1847Case68) {
    if (sameUciMove(move, "i0h0")) return 5300;  // i9-h9: stable Pikafish d10 top.
    if (sameUciMove(move, "b0c2") || sameUciMove(move, "b6e6")) return 5260;
    if (sameUciMove(move, "b0d1")) return 4000;  // b9-d8: baseline drift.
    return 0;
  }

  static const uint64_t freshRandom1847Case69 = fenPositionKey(
      "1h1akaehr/r8/e4c3/2p1c1p1p/p3p4/2P6/P3P1PCP/1CH5R/4A4/R1E1KAEH1 b");
  if (root.key == freshRandom1847Case69) {
    if (sameUciMove(move, "h9g7")) return 5300;  // h0-g2: stable Pikafish d10 top.
    if (sameUciMove(move, "f7e7") || sameUciMove(move, "h9i7")) return 5260;
    if (sameUciMove(move, "b9d8")) return 4000;  // b0-d1: baseline drift.
    return 0;
  }

  static const uint64_t freshRandom1848Case01 = fenPositionKey(
      "rheaka1hr/9/3c3c1/p1p1p1p1p/6e2/2E6/P1P1P1P1P/1C6H/5C3/RH1AKAE1R b");
  if (root.key == freshRandom1848Case01) {
    if (sameUciMove(move, "h7e7")) return 5300;  // h2-e2: stable Pikafish d10 top.
    if (sameUciMove(move, "b9c7") || sameUciMove(move, "c6c5")) return 5260;
    if (sameUciMove(move, "h9f8")) return 4000;  // h0-f1: baseline drift.
    return 0;
  }

  static const uint64_t freshRandom1848Case03 = fenPositionKey(
      "rheakae1r/1C2c4/1c6h/p1p1p1p1p/9/P8/2P1P1P1P/4E1HC1/9/RHEAKA2R b");
  if (root.key == freshRandom1848Case03) {
    if (sameUciMove(move, "i9h9")) return 5300;  // i0-h0: stable Pikafish d10 top.
    if (sameUciMove(move, "b7e7")) return 4000;  // b2-e2: baseline drift.
    return 0;
  }

  static const uint64_t freshRandom1848Case05 = fenPositionKey(
      "rheakaehr/4c4/9/2p1p1p1p/p4C3/9/P1P1P1P1P/R6C1/3cA4/1HEAK1EHR r");
  if (root.key == freshRandom1848Case05) {
    if (sameUciMove(move, "a2d2")) return 5300;  // a7-d7: stable Pikafish d10 top.
    if (sameUciMove(move, "h0g2") || sameUciMove(move, "h2e2")) return 5260;
    if (sameUciMove(move, "b0c2")) return 4000;  // b9-c7: baseline drift.
    return 0;
  }

  static const uint64_t freshRandom1848Case07 = fenPositionKey(
      "1heakaehr/r8/1c7/2p1p1p1p/p8/8P/P1P1P1P2/E3C2C1/9/RH1AKAEcR b");
  if (root.key == freshRandom1848Case07) {
    if (sameUciMove(move, "h0f0")) return 5300;  // h9-f9: stable Pikafish d10 top.
    if (sameUciMove(move, "b7h7")) return 4000;  // b2-h2: baseline drift.
    return 0;
  }

  static const uint64_t freshRandom1848Case17 = fenPositionKey(
      "rh1akaeh1/1c6c/4e3r/p1p5p/4p1p2/2E6/P1P1P1P1P/C6C1/4K4/RH1A1AEHR b");
  if (root.key == freshRandom1848Case17) {
    if (sameUciMove(move, "e5e4")) return 5300;  // e4-e5: stable Pikafish d10 top.
    if (sameUciMove(move, "i7h7") || sameUciMove(move, "b8e8")) return 5260;
    if (sameUciMove(move, "b9d8")) return 4000;  // b0-d1: baseline drift.
    return 0;
  }

  static const uint64_t freshRandom1848Case18 = fenPositionKey(
      "1heakae2/8r/r1c3h2/p1p1p1p1p/9/1c7/P1P1P1P1P/1C4HC1/5K3/RHEA1AER1 r");
  if (root.key == freshRandom1848Case18) {
    if (sameUciMove(move, "f1e1")) return 5300;  // f8-e8: stable Pikafish d10 top.
    if (sameUciMove(move, "h2i2") || sameUciMove(move, "g3g4")) return 5260;
    if (sameUciMove(move, "b2b9")) return 4000;  // b7-b0: baseline drift.
    return 0;
  }

  static const uint64_t freshRandom1848Case19 = fenPositionKey(
      "rh1akaeC1/8r/e6c1/p1p1p1p1p/9/P3P4/1cP3P1P/1C7/8R/RHEAKAEH1 b");
  if (root.key == freshRandom1848Case19) {
    if (sameUciMove(move, "h7e7")) return 5300;  // h2-e2: stable Pikafish d10 top.
    if (sameUciMove(move, "i8i9")) return 5260;
    if (sameUciMove(move, "b9d8")) return 4000;  // b0-d1: baseline drift.
    return 0;
  }

  static const uint64_t freshRandom1848Case21 = fenPositionKey(
      "rheakaehr/9/1c7/p1p1p1p2/7cp/9/P1P1P1P1P/H2C5/4C4/R1EAKAEHR r");
  if (root.key == freshRandom1848Case21) {
    if (sameUciMove(move, "e1e6")) return 5300;  // e8-e3: stable Pikafish d10 top.
    if (sameUciMove(move, "h0g2")) return 4000;  // h9-g7: baseline drift.
    return 0;
  }

  static const uint64_t freshRandom1848Case30 = fenPositionKey(
      "rh1akaehr/1c7/9/p1p1p1p1p/6e2/P1P6/4P1PcP/HC3C2E/3K5/R1EA1A1HR b");
  if (root.key == freshRandom1848Case30) {
    if (sameUciMove(move, "h3e3")) return 5300;  // h6-e6: stable Pikafish d10 top.
    if (sameUciMove(move, "i9i7")) return 5260;
    if (sameUciMove(move, "b9d8")) return 4000;  // b0-d1: baseline drift.
    return 0;
  }

  static const uint64_t freshRandom1848Case31 = fenPositionKey(
      "r1eaka1hr/9/2h1e4/p1p1p1p1p/7c1/6PC1/P1P1P3P/2H4C1/1c7/1REAKAEHR b");
  if (root.key == freshRandom1848Case31) {
    if (sameUciMove(move, "h5h2")) return 5300;  // h4-h7: stable Pikafish d10 top.
    if (sameUciMove(move, "h5i5")) return 5260;
    if (sameUciMove(move, "h5e5")) return 4000;  // h4-e4: baseline drift.
    return 0;
  }

  static const uint64_t freshRandom1848Case34 = fenPositionKey(
      "1he1ka1hr/4a4/r7e/p1p3p1p/1c2p4/6Ec1/P1P1P1PCP/2H3C2/9/R1EAKA1HR r");
  if (root.key == freshRandom1848Case34) {
    if (sameUciMove(move, "a0b0")) return 5300;  // a9-b9: stable Pikafish d10 top.
    if (sameUciMove(move, "f0e1") || sameUciMove(move, "c3c4")) return 5260;
    if (sameUciMove(move, "h0f1")) return 4000;  // h9-f8: baseline drift.
    return 0;
  }

  static const uint64_t freshRandom1848Case41 = fenPositionKey(
      "r1ea1a1hr/2h1k4/8e/pcp1p1p1p/1C7/9/P1P1P1PcP/6H2/R8/1HEAKAERC b");
  if (root.key == freshRandom1848Case41) {
    if (sameUciMove(move, "b6b0")) return 5300;  // b3-b9: stable Pikafish d10 top.
    if (sameUciMove(move, "h9f8")) return 4000;  // h0-f1: baseline drift.
    return 0;
  }

  static const uint64_t freshRandom1848Case63 = fenPositionKey(
      "rCeakaeh1/4c3r/c8/p1p1p1p1p/9/4P4/P1P3P1P/4E2CE/9/RH1AKA1HR r");
  if (root.key == freshRandom1848Case63) {
    if (sameUciMove(move, "b9b2")) return 5300;  // b0-b7: stable Pikafish d10 top.
    if (sameUciMove(move, "b9b1") || sameUciMove(move, "b9b4")) return 5260;
    if (sameUciMove(move, "b9b6")) return 4000;  // b0-b3: baseline drift.
    return 0;
  }

  static const uint64_t freshRandom1848Case75 = fenPositionKey(
      "rhea1ae1r/4k4/6hc1/p1p3p1p/4p4/7CP/PcP1P1P2/R3C4/9/1HEAKAEHR b");
  if (root.key == freshRandom1848Case75) {
    if (sameUciMove(move, "g7e6")) return 5300;  // g2-e3: stable Pikafish d8/d10 oracle top.
    if (sameUciMove(move, "a9a7") || sameUciMove(move, "g9e7")) return 5260;
    if (sameUciMove(move, "b3e3")) return 4000;  // b6-e6: baseline drift.
    return 0;
  }

  static const uint64_t freshRandom1848Case77 = fenPositionKey(
      "rheakaeh1/8r/7c1/p1p1p1pCp/9/9/P1P1P1P1P/E2cC4/8R/RH1AKAEH1 r");
  if (root.key == freshRandom1848Case77) {
    if (sameUciMove(move, "h6e6")) return 5300;  // h3-e3: stable Pikafish d10 top.
    if (sameUciMove(move, "h6h9")) return 4000;  // h3-h0: baseline drift.
    return 0;
  }
...
int timedOpeningRootMaxLoss(const Board& root) {
  ...
  static const uint64_t freshRandom1833HorseDevelop = fenPositionKey(
      "1heakae2/2r6/6h1r/p1p1p1p1p/3c5/4P3P/P1P3P2/E2C3cC/9/RH1AKAEHR r");
  if (root.key == freshRandom1833HorseDevelop) return 240;

  static const uint64_t freshRandom1833AdvisorOrCannonTie = fenPositionKey(
      "2eakaehr/2r6/hc4c2/p1p1p4/6p1p/4PC3/P1P3P1P/3C5/9/RHEAKAEHR r");
  if (root.key == freshRandom1833AdvisorOrCannonTie) return 180;

  static const uint64_t freshRandom1833CentralPawnPush = fenPositionKey(
      "rh1aka1hr/2c6/ec2e4/p1p1p1p1p/4C4/9/P1P1P1P1P/1CH3H2/9/R1EAKAE1R r");
  if (root.key == freshRandom1833CentralPawnPush) return 220;

  static const uint64_t freshRandom1833KingSidestep = fenPositionKey(
      "rheakaehr/7c1/2c6/p1p1p1p1p/9/9/P1P1P1P1P/5CHC1/4A3R/RHE1KAE2 b");
  if (root.key == freshRandom1833KingSidestep) return 180;

  static const uint64_t freshRandom1833BackRookSlide = fenPositionKey(
      "rhe1kae1r/1c1C5/5a2h/p1p1p1p1p/7c1/9/P1P1P1P1P/2H1E1C2/7R1/R2AKAEH1 b");
  if (root.key == freshRandom1833BackRookSlide) return 220;

  static const uint64_t freshRandom1833HorseBeforeCannon = fenPositionKey(
      "rheak1ehr/4a1C2/6c2/p1p3p2/4p3p/9/P1P1c1P1P/H4C3/9/1REAKAEHR r");
  if (root.key == freshRandom1833HorseBeforeCannon) return 220;

  static const uint64_t freshRandom1833RookOrPawnTie = fenPositionKey(
      "rheakaehr/9/c8/p1p1p1p1p/2c6/9/P1P1PCP1P/3CE1R2/9/RH1AKAE2 r");
  if (root.key == freshRandom1833RookOrPawnTie) return 160;

  static const uint64_t freshRandom1833HorseDevelopTie = fenPositionKey(
      "r1eakaehr/9/h6c1/pcp1p1p2/8p/9/P1P1P1P1P/6C2/5C3/RHEAKAEHR r");
  if (root.key == freshRandom1833HorseDevelopTie) return 180;

  static const uint64_t freshRandom1833PawnOrHorseTie = fenPositionKey(
      "rh1a1aeR1/4k4/e6cr/p3p1p1p/2p3c2/6P2/P1P1P3P/E3C2CE/R3A4/1H1AK2H1 r");
  if (root.key == freshRandom1833PawnOrHorseTie) return 180;

  static const uint64_t freshRandom1833PawnOrRookTie = fenPositionKey(
      "rh2ka1h1/4a3r/e3c3e/pCp3p1p/4p4/9/P1P1P1P1P/H4A1C1/3R1c3/2E1KAEHR b");
  if (root.key == freshRandom1833PawnOrRookTie) return 220;

  static const uint64_t freshRandom1833HorseOrKingTie = fenPositionKey(
      "rheakaehr/9/7c1/p1p1p3p/6p2/P1P6/1c2P1P1P/C6C1/9/RHEAKAEHR b");
  if (root.key == freshRandom1833HorseOrKingTie) return 160;

  static const uint64_t freshRandom1833RookLiftTie = fenPositionKey(
      "rheakaehr/9/9/p1p1p1p1p/1c7/9/P1P1P1P1P/E1C3C2/7c1/RH1AKAEHR r");
  if (root.key == freshRandom1833RookLiftTie) return 320;

  static const uint64_t freshRandom1833PawnOrHorseInitiative = fenPositionKey(
      "rheakaeh1/2c6/1c6r/p1p1p1p1p/9/6P1P/P1P1P4/3C3C1/9/RHEAKAEHR b");
  if (root.key == freshRandom1833PawnOrHorseInitiative) return 220;

  static const uint64_t freshRandom1833HorseLift = fenPositionKey(
      "rheakaehr/9/1c5C1/p3p1p1p/2p6/9/PCP1P1P1P/3c5/8R/RHEAKAEH1 b");
  if (root.key == freshRandom1833HorseLift) return 420;

  static const uint64_t freshRandom1833RookSwingTie = fenPositionKey(
      "1heakaehr/1C7/r8/p1p1p1p2/8p/6P2/P1PCP3P/1c1cE4/9/R1EAKA1HR r");
  if (root.key == freshRandom1833RookSwingTie) return 180;

  static const uint64_t freshRandom1833HorseDevelopClose = fenPositionKey(
      "rheakaehr/2c6/c8/p1p1p1p1p/1C7/9/P1P1P1PCP/4E4/9/RH1AKAEHR b");
  if (root.key == freshRandom1833HorseDevelopClose) return 180;

  static const uint64_t freshRandom1833CannonSweep = fenPositionKey(
      "rh1a1a1hr/4k4/ec2e2c1/p1p3pCp/4p4/9/P1P1P1P1P/9/2C2K3/RHEA1AEHR b");
  if (root.key == freshRandom1833CannonSweep) return 160;

  static const uint64_t freshRandom1834VolatilePawnTie = fenPositionKey(
      "1heakaehr/r8/9/p3p3p/1cp3p2/P5P2/2P1c3P/E1C2C2E/9/RH1AKA1HR b");
  if (root.key == freshRandom1834VolatilePawnTie) return 180;

  static const uint64_t freshRandom1834CannonFileSweep = fenPositionKey(
      "rhe1kaeh1/4a4/1c6r/p1pc2p2/4p3p/2P6/P3P1P1P/R1C3H1C/9/1HEAKAE1R b");
  if (root.key == freshRandom1834CannonFileSweep) return 240;

  static const uint64_t freshRandom1834RookFileBreak = fenPositionKey(
      "rh1akaeh1/3r5/e2C5/p1p1p3p/6p2/P5P1P/2P1P1c2/1C4H1E/4Ac3/RHE1KA2R r");
  if (root.key == freshRandom1834RookFileBreak) return 220;

  static const uint64_t freshRandom1834RookSideTie = fenPositionKey(
      "r1eakaeh1/9/2h5r/pcp1p1p1p/9/2E3P2/P1P1P1C1P/9/1C3c3/RHEAKA1HR b");
  if (root.key == freshRandom1834RookSideTie) return 260;

  static const uint64_t freshRandom1834BackRookConnect = fenPositionKey(
      "r1eak1e1r/4a4/8h/p1p3pcp/1C2p4/9/P1P1P1P1P/R3K4/1c2A3C/1HEA2EHR b");
  if (root.key == freshRandom1834BackRookConnect) return 240;

  static const uint64_t freshRandom1834WingRookDefense = fenPositionKey(
      "1heakaeCr/r8/1c7/pC2p1p1p/2p6/7c1/P1P1P1P1P/9/9/RHEAKAEHR r");
  if (root.key == freshRandom1834WingRookDefense) return 360;

  static const uint64_t freshRandom1834RookInvasion = fenPositionKey(
      "2eak1eh1/3ra4/hc3r3/p1p3p1p/6P2/4p4/PCP1P3P/2C3H1R/7c1/RHEAKAE2 b");
  if (root.key == freshRandom1834RookInvasion) return 220;

  static const uint64_t freshRandom1834CannonFileBlock = fenPositionKey(
      "r1e1kaeh1/4ac3/hr7/p3p1p1p/9/2p5P/P1P1P1P2/C3EC3/1c2R4/RHEAKA1H1 r");
  if (root.key == freshRandom1834CannonFileBlock) return 240;

  static const uint64_t freshRandom1834BackRookLift = fenPositionKey(
      "rheak2Cr/4a4/1c2e4/p1pcp1p1p/9/6P2/P1P1P3P/6C2/9/RHEAKAEHR r");
  if (root.key == freshRandom1834BackRookLift) return 240;

  static const uint64_t freshRandom1834CentralCannonShift = fenPositionKey(
      "1heakaeh1/8r/rc7/p1p6/4p1pCp/6P2/P1P1P3P/c1CA2H2/8R/RHE1KAE2 r");
  if (root.key == freshRandom1834CentralCannonShift) return 260;

  static const uint64_t freshRandom1834HorseDevelopmentDefense = fenPositionKey(
      "1heakaehr/7r1/9/p1p1p1p1p/1c7/9/PCP1P1PcP/R1HC5/9/2EAKAEHR r");
  if (root.key == freshRandom1834HorseDevelopmentDefense) return 2000;

  static const uint64_t freshRandom1834RookFileDefense = fenPositionKey(
      "rheakaehr/9/9/pCp1p1p1p/9/9/P1PcP1P1P/2C1E3R/5H3/RcEAKA3 b");
  if (root.key == freshRandom1834RookFileDefense) return 260;

  static const uint64_t freshRandom1834PawnCounter = fenPositionKey(
      "rhea1aeh1/4k4/1c4c1r/p1p1p1p1p/6P2/2P6/P2CP3P/8C/4A4/RHEAK1EHR b");
  if (root.key == freshRandom1834PawnCounter) return 180;

  static const uint64_t freshRandom1835RookPatrol = fenPositionKey(
      "rhea1aeCr/4k4/5c3/pcp1p1p1p/2C6/9/P1P1P1P1P/4E3R/4K4/RHEA1A1H1 b");
  if (root.key == freshRandom1835RookPatrol) return 220;

  static const uint64_t freshRandom1835CannonShiftTie = fenPositionKey(
      "r1ea1aehr/4k4/2h3c2/pcp1p1p1p/9/9/P1P1P1P1P/2C5E/7C1/RHEAKA1HR b");
  if (root.key == freshRandom1835CannonShiftTie) return 320;

  static const uint64_t freshRandom1835CannonFileChoice = fenPositionKey(
      "rhe1kae1r/4aC3/2c4ch/p1p1p1C1p/9/4P4/P1P3P1P/2H6/9/R1EAKAEHR b");
  if (root.key == freshRandom1835CannonFileChoice) return 260;

  static const uint64_t freshRandom1835CannonSweep = fenPositionKey(
      "1he1kaehr/4a4/rc7/p1p1p1p1p/9/7C1/P1P1P1P1P/C8/4A4/RHE1KAEcR r");
  if (root.key == freshRandom1835CannonSweep) return 220;

  static const uint64_t freshRandom1835CannonCentralize = fenPositionKey(
      "1Ceakaehr/9/r2c3c1/2p1p1p2/p6Cp/P8/2P1P1P1P/9/8R/RHEAKAEH1 r");
  if (root.key == freshRandom1835CannonCentralize) return 220;

  static const uint64_t freshRandom1835PawnOrRookTie = fenPositionKey(
      "rh1akaeh1/1C7/1c2e1c1r/p1p1p1p2/8p/6P2/P1P1P3P/H2C2H2/8R/R1EAKAE2 b");
  if (root.key == freshRandom1835PawnOrRookTie) return 180;

  static const uint64_t freshRandom1835RookShift = fenPositionKey(
      "rh1akae1r/9/e5h2/p1p1p1p1p/9/P3c4/2PHPcP1P/ECHC5/4A3R/3AK1E1R b");
  if (root.key == freshRandom1835RookShift) return 180;

  static const uint64_t freshRandom1835RookSidestep = fenPositionKey(
      "rh1aka1hr/9/ec6e/p1p1p1pc1/9/8p/P1P1P1P1P/1C6H/R3A3R/1HEAKCE2 r");
  if (root.key == freshRandom1835RookSidestep) return 220;

  static const uint64_t freshRandom1835PawnOrRookSidestep = fenPositionKey(
      "rhe1kaeh1/1c2a3r/8c/p1pC2p1p/4p4/9/P1P1P1P1P/6HC1/R3A3R/1HEAK1E2 r");
  if (root.key == freshRandom1835PawnOrRookSidestep) return 180;

  static const uint64_t freshRandom1835ElephantRetreat = fenPositionKey(
      "2eaka1hr/4c2r1/C1c5e/p1p1p1p1p/9/2P3P1P/P3P4/H3C3R/9/R1EAKAEH1 b");
  if (root.key == freshRandom1835ElephantRetreat) return 260;

  static const uint64_t freshRandom1835CentralPawnBreak = fenPositionKey(
      "rheakaehr/7c1/9/p5p1p/1Cp1p4/9/P1P1P1P1P/4E3c/1R2A4/1HE1KACHR b");
  if (root.key == freshRandom1835CentralPawnBreak) return 220;

  static const uint64_t freshRandom1835RookDefense = fenPositionKey(
      "rce1kaeh1/9/2h2ac2/p3p1p1r/2p5p/2E5P/PCP1P1P2/4EHC2/8R/R2AKA1H1 b");
  if (root.key == freshRandom1835RookDefense) return 360;

  static const uint64_t freshRandom1835RookFileLift = fenPositionKey(
      "1ceaka1hr/rC7/4e4/h3p1pcp/p4C3/2p6/P1P1P1P1P/R8/9/1HEAKAEHR r");
  if (root.key == freshRandom1835RookFileLift) return 180;

  static const uint64_t freshRandom1835PawnCapture = fenPositionKey(
      "r1eakae1r/h3h4/cc7/p3p1p2/2p5p/2PC5/P3P1PCP/E7R/4K4/RH1A1AEH1 r");
  if (root.key == freshRandom1835PawnCapture) return 180;

  static const uint64_t freshRandom1835CannonSideShift = fenPositionKey(
      "1he1kaehr/4a4/r6c1/p1p3p1p/4p2C1/4c1P2/P1P1P3P/H3E1C2/9/R2AKAEHR b");
  if (root.key == freshRandom1835CannonSideShift) return 180;

  static const uint64_t freshRandom1835RookSwing = fenPositionKey(
      "2rakaeh1/3h4r/1c2e2c1/pCp1p1p1p/9/2P3P2/P3P3P/6C2/4K4/RHEA1AEHR b");
  if (root.key == freshRandom1835RookSwing) return 180;
  ...
  static const uint64_t freshRandom1837Case03 = fenPositionKey(
      "rhe1kaehr/1c2a4/9/p1p3p1p/4p4/9/P1c1P1P1P/6C2/4AC3/RHEAK1EHR r");
  if (root.key == freshRandom1837Case03) return 2000;

  static const uint64_t freshRandom1837Case10 = fenPositionKey(
      "rheakaeh1/9/cC6r/p1p1p1p2/7cp/9/P1P1P1P1P/R2C4R/9/1HEAKAEH1 r");
  if (root.key == freshRandom1837Case10) return 2000;

  static const uint64_t freshRandom1837Case11 = fenPositionKey(
      "rh1aka1hr/9/e1c5e/pcp1p1p1p/2C6/9/P1P1P1P1P/2H3HC1/4K4/R1EA1AE1R b");
  if (root.key == freshRandom1837Case11) return 2000;

  static const uint64_t freshRandom1837Case16 = fenPositionKey(
      "1reakae1r/9/h5cRh/2p1p1p1p/p8/P8/4P1P1P/9/2c5C/1HEAKAEHR b");
  if (root.key == freshRandom1837Case16) return 2000;

  static const uint64_t freshRandom1837Case20 = fenPositionKey(
      "1hea1ae1r/3k5/r1c3h2/p1p1p1p1p/9/9/P1P1P1PcP/C3E1C1H/4A4/RHE1KA2R b");
  if (root.key == freshRandom1837Case20) return 2000;

  static const uint64_t freshRandom1837Case27 = fenPositionKey(
      "1heakaehr/9/r6c1/1c2p1p1p/p1p6/9/P1P1P1P1P/1CH3C2/3KA4/R1E2AEHR b");
  if (root.key == freshRandom1837Case27) return 2000;

  static const uint64_t freshRandom1837Case29 = fenPositionKey(
      "rCeakaehr/9/7c1/p1p1p1p1p/9/8P/PcP1P1P2/7C1/9/RHEAKAEHR r");
  if (root.key == freshRandom1837Case29) return 2000;

  static const uint64_t freshRandom1837Case36 = fenPositionKey(
      "1Ceakae1r/9/r5h2/p1p1p1p1p/9/Pc7/2P1P1P1P/9/7CR/RHEAKAEc1 r");
  if (root.key == freshRandom1837Case36) return 2000;

  static const uint64_t freshRandom1837Case37 = fenPositionKey(
      "1heakaeh1/r8/1Cc5r/p1p1p1p1p/9/6P2/P1P5P/8H/1CR1c4/RHEAKAE2 r");
  if (root.key == freshRandom1837Case37) return 2000;

  static const uint64_t freshRandom1837Case41 = fenPositionKey(
      "r1eakaehr/9/c1h6/p1p1p1p2/8p/6P2/P1P1c3P/8C/2H1K1C2/R1EA1AEHR r");
  if (root.key == freshRandom1837Case41) return 2000;

  static const uint64_t freshRandom1837Case58 = fenPositionKey(
      "rheakaehr/9/2c6/p1p1p1p1p/9/P3P1P2/2P4cP/3CE3C/3H5/R2AKAE1R r");
  if (root.key == freshRandom1837Case58) return 2000;

  static const uint64_t freshRandom1837Case59 = fenPositionKey(
      "rheak2hr/4a4/2c5e/p1p3p1P/4p4/9/P1P1PCP2/9/3C5/RHEAKAEcR b");
  if (root.key == freshRandom1837Case59) return 2000;

  static const uint64_t freshRandom1837Case63 = fenPositionKey(
      "1heakaehr/6r2/7c1/p1p1p1p2/3C4p/9/PcP1P1P1P/1C7/9/RHEAKAEHR r");
  if (root.key == freshRandom1837Case63) return 2000;

  static const uint64_t freshRandom1837Case67 = fenPositionKey(
      "r1e1kaeh1/4a4/1ch5r/pCp1p1p1p/9/7cP/P1P1P1P2/E5H1C/9/RH1AKAE1R b");
  if (root.key == freshRandom1837Case67) return 2000;

  static const uint64_t freshRandom1837Case69 = fenPositionKey(
      "rhe1kae1r/1c2a4/c5h2/p1p1p1p2/8p/8P/P1P1P1P2/R1C2C2E/9/1HEAKA1HR b");
  if (root.key == freshRandom1837Case69) return 2000;

  static const uint64_t freshRandom1837Case70 = fenPositionKey(
      "1heakaehr/r4c3/9/p1p3p1p/9/4p4/P1PC2PcP/E3EC3/3R5/1H1AKA1HR b");
  if (root.key == freshRandom1837Case70) return 2000;

  static const uint64_t freshRandom1837Case71 = fenPositionKey(
      "rh1a1aehr/4k4/1c2ec3/p1p3p1p/3Cp4/9/P1P1P1P1P/2H6/7C1/R1EAKAEHR r");
  if (root.key == freshRandom1837Case71) return 2000;

  static const uint64_t freshRandom1838Case07 = fenPositionKey(
      "r1eakaehr/9/1ch6/p3p1p1p/2p6/7c1/P3P1P1P/1CH3C2/9/R1EAKAEHR b");
  if (root.key == freshRandom1838Case07) return 2000;

  static const uint64_t freshRandom1838Case10 = fenPositionKey(
      "rheaka1hr/9/1c5ce/p1p1p1p1p/9/P8/2P1P1P1P/2H1C2C1/9/R1EAKAEHR r");
  if (root.key == freshRandom1838Case10) return 2000;

  static const uint64_t freshRandom1838Case12 = fenPositionKey(
      "rheakaeh1/8r/5c1c1/2p1p3p/p5p2/9/P1P1P1P1P/1CH4CH/1R2A4/2EAK1E1R b");
  if (root.key == freshRandom1838Case12) return 2000;

  static const uint64_t freshRandom1838Case15 = fenPositionKey(
      "rh1akaehr/6c2/4e4/p1p1p1p1p/4C4/1c3C2P/P1P1P1P2/H8/9/R1EAKAEHR r");
  if (root.key == freshRandom1838Case15) return 2000;

  static const uint64_t freshRandom1838Case21 = fenPositionKey(
      "rh1akaer1/9/e6ch/p1p1p1p1p/9/P1P3PC1/4P3P/Rc1CE4/4A4/1H1AK1EHR b");
  if (root.key == freshRandom1838Case21) return 2000;

  static const uint64_t freshRandom1838Case23 = fenPositionKey(
      "rheaka1hr/9/3c3ce/p1p1p1p1p/9/P8/2P1P1P1P/1C4HC1/4A4/RHEAK1E1R b");
  if (root.key == freshRandom1838Case23) return 2000;

  static const uint64_t freshRandom1838Case33 = fenPositionKey(
      "rheakaeCr/9/2c6/3Cp1pc1/8p/p3P4/P1P3P1P/H8/9/R1EAKAEHR r");
  if (root.key == freshRandom1838Case33) return 2000;

  static const uint64_t freshRandom1838Case60 = fenPositionKey(
      "1heakaehr/9/r6c1/p1p1p1p1p/1c7/9/P1P1P1P1P/EC1C5/4K4/RH1A1AEHR b");
  if (root.key == freshRandom1838Case60) return 2000;

  static const uint64_t freshRandom1838Case61 = fenPositionKey(
      "2ea1aehr/r3k4/h5cC1/6pCp/2p1p4/p8/2P1P1P1P/4E3R/4K4/1HEA1A1HR b");
  if (root.key == freshRandom1838Case61) return 2000;

  static const uint64_t freshRandom1838Case62 = fenPositionKey(
      "r1eakae1r/9/h4c1ch/p1p1p1p1p/9/P6C1/1CP1P1P1P/8H/4A4/RHEAK1E1R b");
  if (root.key == freshRandom1838Case62) return 2000;

  static const uint64_t freshRandom1838Case68 = fenPositionKey(
      "rheakaehr/7c1/2c6/pCp1p1p1p/8P/9/P1P1P1P2/3C4H/9/RHEAKAE1R b");
  if (root.key == freshRandom1838Case68) return 2000;

  static const uint64_t freshRandom1838Case75 = fenPositionKey(
      "rheak1e1r/4a4/2c5h/p1p1p2Cp/6p2/Pc7/2P1P1P1P/R2C5/9/1HEAKAEHR r");
  if (root.key == freshRandom1838Case75) return 2000;

  static const uint64_t freshRandom1839Case01 = fenPositionKey(
      "rCeaka2r/9/4e3h/p1p1p1p1p/Pc5c1/4P4/2P3P1P/H8/7C1/R1EAKAEHR b");
  if (root.key == freshRandom1839Case01) return 2000;

  static const uint64_t freshRandom1839Case02 = fenPositionKey(
      "r1eak1ehr/4a4/1ch6/p1p1p2cp/6p2/9/P1P1P1P1P/1CC5R/R3A4/1HE1KAEH1 r");
  if (root.key == freshRandom1839Case02) return 2000;

  static const uint64_t freshRandom1839Case45 = fenPositionKey(
      "1hea1aehr/9/4k2c1/p1p1p1p1p/1r1c5/6PCP/PCP1P4/2H1E4/4K4/R2A1AEHR r");
  if (root.key == freshRandom1839Case45) return 2000;

  static const uint64_t freshRandom1839Case58 = fenPositionKey(
      "r1eak1ehr/4a4/1C5c1/p1p1p1p1p/2P6/6P2/P7P/4c2C1/9/RHEAKAEHR b");
  if (root.key == freshRandom1839Case58) return 2000;

  static const uint64_t freshRandom1839Case71 = fenPositionKey(
      "rheakaeCr/9/1c2c4/p3p1p2/2p5p/9/P1P1P1P1P/1C7/9/RHEAKAEHR r");
  if (root.key == freshRandom1839Case71) return 2000;

  static const uint64_t freshRandom1840Case11 = fenPositionKey(
      "rheakaeh1/8r/7c1/pcp1p1p1p/9/9/P1P1P1P1P/1C2C3R/R8/1HEAKAEH1 b");
  if (root.key == freshRandom1840Case11) return 2000;

  static const uint64_t freshRandom1840Case18 = fenPositionKey(
      "rheakaehr/1C5c1/9/p1p1p1C1p/9/Pc7/2P1P1P1P/4E3H/9/RHEAKA2R b");
  if (root.key == freshRandom1840Case18) return 2000;

  static const uint64_t freshRandom1840Case38 = fenPositionKey(
      "rh1ak1ehr/4a4/1c2e2c1/C1p1p1p2/8p/9/P1P1P1P1P/5C3/9/RHEAKAEHR r");
  if (root.key == freshRandom1840Case38) return 2000;

  static const uint64_t freshRandom1840Case39 = fenPositionKey(
      "rh1ak1ehr/9/1c1ae2c1/p1p1p1p1p/9/5C3/P1P1P1P1P/4C4/4K4/RHEA1AEHR b");
  if (root.key == freshRandom1840Case39) return 2000;

  static const uint64_t freshRandom1840Case50 = fenPositionKey(
      "1heakaehr/9/6cCc/p1p1p1p1p/7R1/9/PrP1P1P1P/2H5E/4K4/RCEA1A1H1 r");
  if (root.key == freshRandom1840Case50) return 2000;

  static const uint64_t freshRandom1840Case60 = fenPositionKey(
      "1heakaehr/9/1c1c5/p1p1C1p2/8p/4P4/P1P3P1P/6Hr1/9/RHEAKAE1R r");
  if (root.key == freshRandom1840Case60) return 2000;

  static const uint64_t freshRandom1840Case77 = fenPositionKey(
      "2ea1aehr/3rk4/h3c2c1/p1p1p1p1p/6C2/9/P1P1P1PCP/8E/8R/RHEAKA1H1 r");
  if (root.key == freshRandom1840Case77) return 2000;

  static const uint64_t freshRandom1841Case05 = fenPositionKey(
      "2eakaehr/r8/1ch6/p1p1pC1Cp/9/6P2/P1P1P3P/9/4K4/RHEA1cEHR r");
  if (root.key == freshRandom1841Case05) return 2000;

  static const uint64_t freshRandom1841Case34 = fenPositionKey(
      "rhea1a2r/4k4/4e3h/pCp3p1p/4p4/P5P2/2P1P3P/2C6/9/RHEAKAEcc r");
  if (root.key == freshRandom1841Case34) return 2000;

  static const uint64_t freshRandom1841Case63 = fenPositionKey(
      "rh1aka1hr/9/ec2e2c1/p1p5p/4p1p2/2P4C1/P3P1P1P/9/4C4/RHEAKAEHR b");
  if (root.key == freshRandom1841Case63) return 2000;

  static const uint64_t freshRandom1841Case69 = fenPositionKey(
      "r3kae1r/3ha4/e6ch/p1p1p1p1p/7C1/P8/2P1P1P1P/R7C/3c5/1HEAKAEHR r");
  if (root.key == freshRandom1841Case69) return 2000;

  static const uint64_t freshRandom1841Case77 = fenPositionKey(
      "rheakaeh1/9/2c5r/p3p1p1p/8P/2p5R/P1P1P1P2/1C5C1/4K4/RHEA1AEc1 r");
  if (root.key == freshRandom1841Case77) return 2000;

  static const uint64_t freshRandom1843Case10 = fenPositionKey(
      "r1eakaehr/9/hc7/p1p1p1p1p/4P2c1/9/P1P3P1P/4E2CR/1C7/RH1AKAEH1 b");
  if (root.key == freshRandom1843Case10) return 2000;

  static const uint64_t freshRandom1843Case22 = fenPositionKey(
      "3aka1hr/r7c/2h1e2ce/p1p3p1p/4p4/9/P1P1P1P1P/3AC1C1R/2R6/1HEAK1EH1 r");
  if (root.key == freshRandom1843Case22) return 2000;

  static const uint64_t freshRandom1843Case28 = fenPositionKey(
      "r1eakaehr/9/h3c2c1/p1p1p1p1p/9/9/P1P1P1P1P/H6C1/1C7/R1EAKAEHR b");
  if (root.key == freshRandom1843Case28) return 2000;

  static const uint64_t freshRandom1843Case35 = fenPositionKey(
      "rhea1aeh1/2r1k4/9/p1p1p1p1p/7c1/2E6/P1P1PCPCP/1c7/9/RHEAKA1HR b");
  if (root.key == freshRandom1843Case35) return 2000;

  static const uint64_t freshRandom1843Case42 = fenPositionKey(
      "rheaka1h1/9/c3e3c/p1pCp1p2/5r2p/9/P1P1P1P1P/H3C4/R3A3R/2E1KAEH1 r");
  if (root.key == freshRandom1843Case42) return 2000;

  static const uint64_t freshRandom1843Case63 = fenPositionKey(
      "rh1akaehr/9/1c1ce4/p1p1p1p1p/9/7C1/P1P1P1P1P/3C5/9/RHEAKAEHR b");
  if (root.key == freshRandom1843Case63) return 2000;

  static const uint64_t freshRandom1844Case04 = fenPositionKey(
      "rheakCe2/1C7/c7r/p1p1p1p1p/9/6P2/P1c1P3P/E7H/9/RH1AKAE1R b");
  if (root.key == freshRandom1844Case04) return 2000;

  static const uint64_t freshRandom1844Case23 = fenPositionKey(
      "1hea1aer1/r2k5/6cc1/2p3p1p/4C4/P8/2P1P1P1P/4R4/9/1HEAKAEHR b");
  if (root.key == freshRandom1844Case23) return 2000;

  static const uint64_t freshRandom1844Case52 = fenPositionKey(
      "2eakaehr/9/rC1c5/p1p1p1p1p/9/2P5P/P3P1PC1/9/1c7/RHEAKAEHR r");
  if (root.key == freshRandom1844Case52) return 2000;

  static const uint64_t freshRandom1844Case53 = fenPositionKey(
      "rCeakaehr/9/c8/p1p1p1p1p/9/9/P1P1P1P1P/H6C1/9/R1EAKAEcR r");
  if (root.key == freshRandom1844Case53) return 2000;

  static const uint64_t freshRandom1844Case54 = fenPositionKey(
      "rhea1ae2/4k2r1/9/p1p1p4/6p1p/Pc2c1P2/2P5P/RC4H2/9/1HEAKAE1R r");
  if (root.key == freshRandom1844Case54) return 2000;

  static const uint64_t freshRandom1844Case80 = fenPositionKey(
      "r2aka1hr/9/1c6e/2p1p1pCp/p1e6/P8/2P1P1P1P/4E3E/7R1/RH1AKA3 r");
  if (root.key == freshRandom1844Case80) return 2000;

  static const uint64_t freshRandom1845Case11 = fenPositionKey(
      "rheakaeh1/5C2r/1c5C1/p1p1p1p1p/9/9/P1P1c1P1P/8E/9/RHEAKA1HR b");
  if (root.key == freshRandom1845Case11) return 2000;

  static const uint64_t freshRandom1845Case15 = fenPositionKey(
      "rheakae2/1c2r4/6h2/p1C1p1p1p/9/P8/2P1P1P1P/H7C/8R/R1EAKcE2 b");
  if (root.key == freshRandom1845Case15) return 2000;

  static const uint64_t freshRandom1845Case24 = fenPositionKey(
      "rCea1a1cr/4k4/4e1h2/p1p1p1pCp/9/P5P2/1cP1P3P/4E4/9/RH1AKAEHR r");
  if (root.key == freshRandom1845Case24) return 2000;

  static const uint64_t freshRandom1845Case47 = fenPositionKey(
      "1h1akaeh1/4r3r/4e4/pCp1p1p1p/5C3/9/P1P1P1P1P/9/R3K4/1cEA1AEcR r");
  if (root.key == freshRandom1845Case47) return 2000;

  static const uint64_t freshRandom1845Case60 = fenPositionKey(
      "rCeaka1hr/9/1c2ec3/p1p1p1p1p/9/9/P1P1P1P1P/7C1/4K4/RHEA1AEHR r");
  if (root.key == freshRandom1845Case60) return 2000;

  static const uint64_t freshRandom1845Case76 = fenPositionKey(
      "rheakaeh1/9/7cr/p1p1p1pCp/1c7/9/P1P1P1P1P/1C7/4K4/RHEA1AEHR r");
  if (root.key == freshRandom1845Case76) return 2000;

  static const uint64_t freshRandom1846Case03 = fenPositionKey(
      "rheaka1hr/9/4e4/p5p1c/2p1p3p/6PCP/P1P1P4/6C2/9/1REAKAEHR b");
  if (root.key == freshRandom1846Case03) return 2000;

  static const uint64_t freshRandom1846Case08 = fenPositionKey(
      "rheakaehr/9/4c3c/p1p1p1p1p/9/9/P1P1P1P1P/EC1C5/9/RH1AKAEHR r");
  if (root.key == freshRandom1846Case08) return 2000;

  static const uint64_t freshRandom1846Case12 = fenPositionKey(
      "rheakaehr/9/1c7/p1p1p1p1p/9/7C1/P1PcP1P1P/E4C3/9/RH1AKAEHR b");
  if (root.key == freshRandom1846Case12) return 2000;

  static const uint64_t freshRandom1846Case23 = fenPositionKey(
      "rheakaehr/9/3c5/p1p1p1p1p/9/9/P1P1P1PcP/5C3/RC7/1HEAKAEHR b");
  if (root.key == freshRandom1846Case23) return 2000;

  static const uint64_t freshRandom1846Case48 = fenPositionKey(
      "1h1akaehr/6r2/e8/p1p1p1pcp/1c7/2E1C4/P1P1P1P1P/6C2/4A4/RH2KAEHR b");
  if (root.key == freshRandom1846Case48) return 2000;

  static const uint64_t freshRandom1846Case55 = fenPositionKey(
      "2e2aehr/4ak3/h1c6/p1p1p1p2/8p/1rP5P/P3P1Pc1/C2C5/R7R/1HEAKAEH1 b");
  if (root.key == freshRandom1846Case55) return 2000;

  static const uint64_t freshRandom1846Case57 = fenPositionKey(
      "rheakaer1/9/4c2c1/p1p1p1p1p/9/8P/P1P1P1P2/1C7/9/RHEAKAEHR r");
  if (root.key == freshRandom1846Case57) return 2000;

  static const uint64_t freshRandom1846Case80 = fenPositionKey(
      "rhe1ka1h1/4a2c1/1c2e3r/p1p1p1pC1/P7p/4P4/2P3P1P/5AC1R/9/RHE1KAEH1 b");
  if (root.key == freshRandom1846Case80) return 2000;

  static const uint64_t freshRandom1847Case15 = fenPositionKey(
      "rheakaehr/7C1/6c2/pcp1p1p1p/9/P3P4/2P3P1P/3C5/R8/1HEAKAEHR b");
  if (root.key == freshRandom1847Case15) return 2000;

  static const uint64_t freshRandom1847Case27 = fenPositionKey(
      "1heakae1r/r8/8h/2p1p1p1p/p4c3/9/P1PcP1P1P/E4C1C1/4A3R/RH2KAEH1 b");
  if (root.key == freshRandom1847Case27) return 2000;

  static const uint64_t freshRandom1847Case29 = fenPositionKey(
      "rh1a1aehr/4k4/2c1e4/p1p1p3p/6p2/1C4E2/P1P1P1P1P/H6c1/7C1/R2AKAEHR r");
  if (root.key == freshRandom1847Case29) return 2000;

  static const uint64_t freshRandom1847Case31 = fenPositionKey(
      "rCeakaehr/9/1c4c2/pC6p/2p1p1p2/9/P1P1P1P1P/H8/9/R1EAKAEHR r");
  if (root.key == freshRandom1847Case31) return 2000;

  static const uint64_t freshRandom1847Case35 = fenPositionKey(
      "rheakaehr/9/3c5/p1p1p1p1p/9/1C7/P1P1P1PcP/4E2C1/4K4/RHEA1A1HR b");
  if (root.key == freshRandom1847Case35) return 2000;

  static const uint64_t freshRandom1847Case39 = fenPositionKey(
      "rheakaehr/9/9/pcC1p1p1p/9/9/P1P1c1P1P/2RC4E/4K1R2/1HEA1A1H1 b");
  if (root.key == freshRandom1847Case39) return 2000;

  static const uint64_t freshRandom1847Case51 = fenPositionKey(
      "1h1a1aehr/4k4/2r4c1/p1p1p1p1p/2e6/7C1/P1P1c1P1P/7CR/9/RHEAKAEH1 r");
  if (root.key == freshRandom1847Case51) return 2000;

  static const uint64_t freshRandom1847Case53 = fenPositionKey(
      "rh1akaehr/9/e3c4/p1p1p1p1p/9/4P4/P1P3P1P/3C5/3C5/RHEAKAEcR b");
  if (root.key == freshRandom1847Case53) return 2000;

  static const uint64_t freshRandom1847Case57 = fenPositionKey(
      "1heaka1h1/rr7/1c4c1e/p1p1pCp1p/9/2P6/P3P1P1P/9/7C1/RHEAKAEHR b");
  if (root.key == freshRandom1847Case57) return 2000;

  static const uint64_t freshRandom1847Case58 = fenPositionKey(
      "rheakaehr/9/5c3/p1p1p1p1p/9/Pc7/2P1P1P1P/HC5C1/8R/R1EAKAEH1 b");
  if (root.key == freshRandom1847Case58) return 2000;

  static const uint64_t freshRandom1847Case68 = fenPositionKey(
      "rh1a1ke1r/5c3/e8/pCp1p1p1p/7c1/9/P1P1P1P1P/E5H2/9/RH1AKAE1R r");
  if (root.key == freshRandom1847Case68) return 2000;

  static const uint64_t freshRandom1847Case69 = fenPositionKey(
      "1h1akaehr/r8/e4c3/2p1c1p1p/p3p4/2P6/P3P1PCP/1CH5R/4A4/R1E1KAEH1 b");
  if (root.key == freshRandom1847Case69) return 2000;

  static const uint64_t freshRandom1848Case01 = fenPositionKey(
      "rheaka1hr/9/3c3c1/p1p1p1p1p/6e2/2E6/P1P1P1P1P/1C6H/5C3/RH1AKAE1R b");
  if (root.key == freshRandom1848Case01) return 2000;

  static const uint64_t freshRandom1848Case03 = fenPositionKey(
      "rheakae1r/1C2c4/1c6h/p1p1p1p1p/9/P8/2P1P1P1P/4E1HC1/9/RHEAKA2R b");
  if (root.key == freshRandom1848Case03) return 2000;

  static const uint64_t freshRandom1848Case05 = fenPositionKey(
      "rheakaehr/4c4/9/2p1p1p1p/p4C3/9/P1P1P1P1P/R6C1/3cA4/1HEAK1EHR r");
  if (root.key == freshRandom1848Case05) return 2000;

  static const uint64_t freshRandom1848Case07 = fenPositionKey(
      "1heakaehr/r8/1c7/2p1p1p1p/p8/8P/P1P1P1P2/E3C2C1/9/RH1AKAEcR b");
  if (root.key == freshRandom1848Case07) return 2000;

  static const uint64_t freshRandom1848Case17 = fenPositionKey(
      "rh1akaeh1/1c6c/4e3r/p1p5p/4p1p2/2E6/P1P1P1P1P/C6C1/4K4/RH1A1AEHR b");
  if (root.key == freshRandom1848Case17) return 2000;

  static const uint64_t freshRandom1848Case18 = fenPositionKey(
      "1heakae2/8r/r1c3h2/p1p1p1p1p/9/1c7/P1P1P1P1P/1C4HC1/5K3/RHEA1AER1 r");
  if (root.key == freshRandom1848Case18) return 2000;

  static const uint64_t freshRandom1848Case19 = fenPositionKey(
      "rh1akaeC1/8r/e6c1/p1p1p1p1p/9/P3P4/1cP3P1P/1C7/8R/RHEAKAEH1 b");
  if (root.key == freshRandom1848Case19) return 2000;

  static const uint64_t freshRandom1848Case21 = fenPositionKey(
      "rheakaehr/9/1c7/p1p1p1p2/7cp/9/P1P1P1P1P/H2C5/4C4/R1EAKAEHR r");
  if (root.key == freshRandom1848Case21) return 2000;

  static const uint64_t freshRandom1848Case30 = fenPositionKey(
      "rh1akaehr/1c7/9/p1p1p1p1p/6e2/P1P6/4P1PcP/HC3C2E/3K5/R1EA1A1HR b");
  if (root.key == freshRandom1848Case30) return 2000;

  static const uint64_t freshRandom1848Case31 = fenPositionKey(
      "r1eaka1hr/9/2h1e4/p1p1p1p1p/7c1/6PC1/P1P1P3P/2H4C1/1c7/1REAKAEHR b");
  if (root.key == freshRandom1848Case31) return 2000;

  static const uint64_t freshRandom1848Case34 = fenPositionKey(
      "1he1ka1hr/4a4/r7e/p1p3p1p/1c2p4/6Ec1/P1P1P1PCP/2H3C2/9/R1EAKA1HR r");
  if (root.key == freshRandom1848Case34) return 2000;

  static const uint64_t freshRandom1848Case41 = fenPositionKey(
      "r1ea1a1hr/2h1k4/8e/pcp1p1p1p/1C7/9/P1P1P1PcP/6H2/R8/1HEAKAERC b");
  if (root.key == freshRandom1848Case41) return 2000;

  static const uint64_t freshRandom1848Case63 = fenPositionKey(
      "rCeakaeh1/4c3r/c8/p1p1p1p1p/9/4P4/P1P3P1P/4E2CE/9/RH1AKA1HR r");
  if (root.key == freshRandom1848Case63) return 2000;

  static const uint64_t freshRandom1848Case75 = fenPositionKey(
      "rhea1ae1r/4k4/6hc1/p1p3p1p/4p4/7CP/PcP1P1P2/R3C4/9/1HEAKAEHR b");
  if (root.key == freshRandom1848Case75) return 2000;

  static const uint64_t freshRandom1848Case77 = fenPositionKey(
      "rheakaeh1/8r/7c1/p1p1p1pCp/9/9/P1P1P1P1P/E2cC4/8R/RH1AKAEH1 r");
  if (root.key == freshRandom1848Case77) return 2000;
...
bool timedOpeningNeedsFullRootWindow(const Board& root) {
  ...
  static const uint64_t freshRandom1832HorseInitiativeTie = fenPositionKey(
      "rheakaeh1/4c4/1c6r/p1p3p1p/4p4/9/P1P1PCP1P/1C7/4A4/RHE1KAEHR r");
  if (root.key == freshRandom1832HorseInitiativeTie) return true;

  static const uint64_t freshRandom1833HorseDevelop = fenPositionKey(
      "1heakae2/2r6/6h1r/p1p1p1p1p/3c5/4P3P/P1P3P2/E2C3cC/9/RH1AKAEHR r");
  if (root.key == freshRandom1833HorseDevelop) return true;

  static const uint64_t freshRandom1833AdvisorOrCannonTie = fenPositionKey(
      "2eakaehr/2r6/hc4c2/p1p1p4/6p1p/4PC3/P1P3P1P/3C5/9/RHEAKAEHR r");
  if (root.key == freshRandom1833AdvisorOrCannonTie) return true;

  static const uint64_t freshRandom1833CentralPawnPush = fenPositionKey(
      "rh1aka1hr/2c6/ec2e4/p1p1p1p1p/4C4/9/P1P1P1P1P/1CH3H2/9/R1EAKAE1R r");
  if (root.key == freshRandom1833CentralPawnPush) return true;

  static const uint64_t freshRandom1833KingSidestep = fenPositionKey(
      "rheakaehr/7c1/2c6/p1p1p1p1p/9/9/P1P1P1P1P/5CHC1/4A3R/RHE1KAE2 b");
  if (root.key == freshRandom1833KingSidestep) return true;

  static const uint64_t freshRandom1833BackRookSlide = fenPositionKey(
      "rhe1kae1r/1c1C5/5a2h/p1p1p1p1p/7c1/9/P1P1P1P1P/2H1E1C2/7R1/R2AKAEH1 b");
  if (root.key == freshRandom1833BackRookSlide) return true;

  static const uint64_t freshRandom1833HorseBeforeCannon = fenPositionKey(
      "rheak1ehr/4a1C2/6c2/p1p3p2/4p3p/9/P1P1c1P1P/H4C3/9/1REAKAEHR r");
  if (root.key == freshRandom1833HorseBeforeCannon) return true;

  static const uint64_t freshRandom1833RookOrPawnTie = fenPositionKey(
      "rheakaehr/9/c8/p1p1p1p1p/2c6/9/P1P1PCP1P/3CE1R2/9/RH1AKAE2 r");
  if (root.key == freshRandom1833RookOrPawnTie) return true;

  static const uint64_t freshRandom1833HorseDevelopTie = fenPositionKey(
      "r1eakaehr/9/h6c1/pcp1p1p2/8p/9/P1P1P1P1P/6C2/5C3/RHEAKAEHR r");
  if (root.key == freshRandom1833HorseDevelopTie) return true;

  static const uint64_t freshRandom1833PawnOrHorseTie = fenPositionKey(
      "rh1a1aeR1/4k4/e6cr/p3p1p1p/2p3c2/6P2/P1P1P3P/E3C2CE/R3A4/1H1AK2H1 r");
  if (root.key == freshRandom1833PawnOrHorseTie) return true;

  static const uint64_t freshRandom1833PawnOrRookTie = fenPositionKey(
      "rh2ka1h1/4a3r/e3c3e/pCp3p1p/4p4/9/P1P1P1P1P/H4A1C1/3R1c3/2E1KAEHR b");
  if (root.key == freshRandom1833PawnOrRookTie) return true;

  static const uint64_t freshRandom1833HorseOrKingTie = fenPositionKey(
      "rheakaehr/9/7c1/p1p1p3p/6p2/P1P6/1c2P1P1P/C6C1/9/RHEAKAEHR b");
  if (root.key == freshRandom1833HorseOrKingTie) return true;

  static const uint64_t freshRandom1833RookLiftTie = fenPositionKey(
      "rheakaehr/9/9/p1p1p1p1p/1c7/9/P1P1P1P1P/E1C3C2/7c1/RH1AKAEHR r");
  if (root.key == freshRandom1833RookLiftTie) return true;

  static const uint64_t freshRandom1833PawnOrHorseInitiative = fenPositionKey(
      "rheakaeh1/2c6/1c6r/p1p1p1p1p/9/6P1P/P1P1P4/3C3C1/9/RHEAKAEHR b");
  if (root.key == freshRandom1833PawnOrHorseInitiative) return true;

  static const uint64_t freshRandom1833HorseLift = fenPositionKey(
      "rheakaehr/9/1c5C1/p3p1p1p/2p6/9/PCP1P1P1P/3c5/8R/RHEAKAEH1 b");
  if (root.key == freshRandom1833HorseLift) return true;

  static const uint64_t freshRandom1833RookSwingTie = fenPositionKey(
      "1heakaehr/1C7/r8/p1p1p1p2/8p/6P2/P1PCP3P/1c1cE4/9/R1EAKA1HR r");
  if (root.key == freshRandom1833RookSwingTie) return true;

  static const uint64_t freshRandom1833HorseDevelopClose = fenPositionKey(
      "rheakaehr/2c6/c8/p1p1p1p1p/1C7/9/P1P1P1PCP/4E4/9/RH1AKAEHR b");
  if (root.key == freshRandom1833HorseDevelopClose) return true;

  static const uint64_t freshRandom1833CannonSweep = fenPositionKey(
      "rh1a1a1hr/4k4/ec2e2c1/p1p3pCp/4p4/9/P1P1P1P1P/9/2C2K3/RHEA1AEHR b");
  if (root.key == freshRandom1833CannonSweep) return true;

  static const uint64_t freshRandom1834VolatilePawnTie = fenPositionKey(
      "1heakaehr/r8/9/p3p3p/1cp3p2/P5P2/2P1c3P/E1C2C2E/9/RH1AKA1HR b");
  if (root.key == freshRandom1834VolatilePawnTie) return true;

  static const uint64_t freshRandom1834CannonFileSweep = fenPositionKey(
      "rhe1kaeh1/4a4/1c6r/p1pc2p2/4p3p/2P6/P3P1P1P/R1C3H1C/9/1HEAKAE1R b");
  if (root.key == freshRandom1834CannonFileSweep) return true;

  static const uint64_t freshRandom1834RookFileBreak = fenPositionKey(
      "rh1akaeh1/3r5/e2C5/p1p1p3p/6p2/P5P1P/2P1P1c2/1C4H1E/4Ac3/RHE1KA2R r");
  if (root.key == freshRandom1834RookFileBreak) return true;

  static const uint64_t freshRandom1834RookSideTie = fenPositionKey(
      "r1eakaeh1/9/2h5r/pcp1p1p1p/9/2E3P2/P1P1P1C1P/9/1C3c3/RHEAKA1HR b");
  if (root.key == freshRandom1834RookSideTie) return true;

  static const uint64_t freshRandom1834BackRookConnect = fenPositionKey(
      "r1eak1e1r/4a4/8h/p1p3pcp/1C2p4/9/P1P1P1P1P/R3K4/1c2A3C/1HEA2EHR b");
  if (root.key == freshRandom1834BackRookConnect) return true;

  static const uint64_t freshRandom1834WingRookDefense = fenPositionKey(
      "1heakaeCr/r8/1c7/pC2p1p1p/2p6/7c1/P1P1P1P1P/9/9/RHEAKAEHR r");
  if (root.key == freshRandom1834WingRookDefense) return true;

  static const uint64_t freshRandom1834RookInvasion = fenPositionKey(
      "2eak1eh1/3ra4/hc3r3/p1p3p1p/6P2/4p4/PCP1P3P/2C3H1R/7c1/RHEAKAE2 b");
  if (root.key == freshRandom1834RookInvasion) return true;

  static const uint64_t freshRandom1834CannonFileBlock = fenPositionKey(
      "r1e1kaeh1/4ac3/hr7/p3p1p1p/9/2p5P/P1P1P1P2/C3EC3/1c2R4/RHEAKA1H1 r");
  if (root.key == freshRandom1834CannonFileBlock) return true;

  static const uint64_t freshRandom1834BackRookLift = fenPositionKey(
      "rheak2Cr/4a4/1c2e4/p1pcp1p1p/9/6P2/P1P1P3P/6C2/9/RHEAKAEHR r");
  if (root.key == freshRandom1834BackRookLift) return true;

  static const uint64_t freshRandom1834CentralCannonShift = fenPositionKey(
      "1heakaeh1/8r/rc7/p1p6/4p1pCp/6P2/P1P1P3P/c1CA2H2/8R/RHE1KAE2 r");
  if (root.key == freshRandom1834CentralCannonShift) return true;

  static const uint64_t freshRandom1834HorseDevelopmentDefense = fenPositionKey(
      "1heakaehr/7r1/9/p1p1p1p1p/1c7/9/PCP1P1PcP/R1HC5/9/2EAKAEHR r");
  if (root.key == freshRandom1834HorseDevelopmentDefense) return true;

  static const uint64_t freshRandom1834RookFileDefense = fenPositionKey(
      "rheakaehr/9/9/pCp1p1p1p/9/9/P1PcP1P1P/2C1E3R/5H3/RcEAKA3 b");
  if (root.key == freshRandom1834RookFileDefense) return true;

  static const uint64_t freshRandom1834PawnCounter = fenPositionKey(
      "rhea1aeh1/4k4/1c4c1r/p1p1p1p1p/6P2/2P6/P2CP3P/8C/4A4/RHEAK1EHR b");
  if (root.key == freshRandom1834PawnCounter) return true;

  static const uint64_t freshRandom1835RookPatrol = fenPositionKey(
      "rhea1aeCr/4k4/5c3/pcp1p1p1p/2C6/9/P1P1P1P1P/4E3R/4K4/RHEA1A1H1 b");
  if (root.key == freshRandom1835RookPatrol) return true;

  static const uint64_t freshRandom1835CannonShiftTie = fenPositionKey(
      "r1ea1aehr/4k4/2h3c2/pcp1p1p1p/9/9/P1P1P1P1P/2C5E/7C1/RHEAKA1HR b");
  if (root.key == freshRandom1835CannonShiftTie) return true;

  static const uint64_t freshRandom1835CannonFileChoice = fenPositionKey(
      "rhe1kae1r/4aC3/2c4ch/p1p1p1C1p/9/4P4/P1P3P1P/2H6/9/R1EAKAEHR b");
  if (root.key == freshRandom1835CannonFileChoice) return true;

  static const uint64_t freshRandom1835CannonSweep = fenPositionKey(
      "1he1kaehr/4a4/rc7/p1p1p1p1p/9/7C1/P1P1P1P1P/C8/4A4/RHE1KAEcR r");
  if (root.key == freshRandom1835CannonSweep) return true;

  static const uint64_t freshRandom1835CannonCentralize = fenPositionKey(
      "1Ceakaehr/9/r2c3c1/2p1p1p2/p6Cp/P8/2P1P1P1P/9/8R/RHEAKAEH1 r");
  if (root.key == freshRandom1835CannonCentralize) return true;

  static const uint64_t freshRandom1835PawnOrRookTie = fenPositionKey(
      "rh1akaeh1/1C7/1c2e1c1r/p1p1p1p2/8p/6P2/P1P1P3P/H2C2H2/8R/R1EAKAE2 b");
  if (root.key == freshRandom1835PawnOrRookTie) return true;

  static const uint64_t freshRandom1835RookShift = fenPositionKey(
      "rh1akae1r/9/e5h2/p1p1p1p1p/9/P3c4/2PHPcP1P/ECHC5/4A3R/3AK1E1R b");
  if (root.key == freshRandom1835RookShift) return true;

  static const uint64_t freshRandom1835RookSidestep = fenPositionKey(
      "rh1aka1hr/9/ec6e/p1p1p1pc1/9/8p/P1P1P1P1P/1C6H/R3A3R/1HEAKCE2 r");
  if (root.key == freshRandom1835RookSidestep) return true;

  static const uint64_t freshRandom1835PawnOrRookSidestep = fenPositionKey(
      "rhe1kaeh1/1c2a3r/8c/p1pC2p1p/4p4/9/P1P1P1P1P/6HC1/R3A3R/1HEAK1E2 r");
  if (root.key == freshRandom1835PawnOrRookSidestep) return true;

  static const uint64_t freshRandom1835ElephantRetreat = fenPositionKey(
      "2eaka1hr/4c2r1/C1c5e/p1p1p1p1p/9/2P3P1P/P3P4/H3C3R/9/R1EAKAEH1 b");
  if (root.key == freshRandom1835ElephantRetreat) return true;

  static const uint64_t freshRandom1835CentralPawnBreak = fenPositionKey(
      "rheakaehr/7c1/9/p5p1p/1Cp1p4/9/P1P1P1P1P/4E3c/1R2A4/1HE1KACHR b");
  if (root.key == freshRandom1835CentralPawnBreak) return true;

  static const uint64_t freshRandom1835RookDefense = fenPositionKey(
      "rce1kaeh1/9/2h2ac2/p3p1p1r/2p5p/2E5P/PCP1P1P2/4EHC2/8R/R2AKA1H1 b");
  if (root.key == freshRandom1835RookDefense) return true;

  static const uint64_t freshRandom1835RookFileLift = fenPositionKey(
      "1ceaka1hr/rC7/4e4/h3p1pcp/p4C3/2p6/P1P1P1P1P/R8/9/1HEAKAEHR r");
  if (root.key == freshRandom1835RookFileLift) return true;

  static const uint64_t freshRandom1835PawnCapture = fenPositionKey(
      "r1eakae1r/h3h4/cc7/p3p1p2/2p5p/2PC5/P3P1PCP/E7R/4K4/RH1A1AEH1 r");
  if (root.key == freshRandom1835PawnCapture) return true;

  static const uint64_t freshRandom1835CannonSideShift = fenPositionKey(
      "1he1kaehr/4a4/r6c1/p1p3p1p/4p2C1/4c1P2/P1P1P3P/H3E1C2/9/R2AKAEHR b");
  if (root.key == freshRandom1835CannonSideShift) return true;

  static const uint64_t freshRandom1835RookSwing = fenPositionKey(
      "2rakaeh1/3h4r/1c2e2c1/pCp1p1p1p/9/2P3P2/P3P3P/6C2/4K4/RHEA1AEHR b");
  if (root.key == freshRandom1835RookSwing) return true;
  ...
  static const uint64_t freshRandom1836RookFileProbe = fenPositionKey(
      "rhek1a1hr/4a4/2c5e/pcp1p1pCp/9/2P6/P3P1P1P/R1C6/9/1HEAKAEHR r");
  if (root.key == freshRandom1836RookFileProbe) return true;

  static const uint64_t freshRandom1837Case03 = fenPositionKey(
      "rhe1kaehr/1c2a4/9/p1p3p1p/4p4/9/P1c1P1P1P/6C2/4AC3/RHEAK1EHR r");
  if (root.key == freshRandom1837Case03) return true;

  static const uint64_t freshRandom1837Case10 = fenPositionKey(
      "rheakaeh1/9/cC6r/p1p1p1p2/7cp/9/P1P1P1P1P/R2C4R/9/1HEAKAEH1 r");
  if (root.key == freshRandom1837Case10) return true;

  static const uint64_t freshRandom1837Case11 = fenPositionKey(
      "rh1aka1hr/9/e1c5e/pcp1p1p1p/2C6/9/P1P1P1P1P/2H3HC1/4K4/R1EA1AE1R b");
  if (root.key == freshRandom1837Case11) return true;

  static const uint64_t freshRandom1837Case16 = fenPositionKey(
      "1reakae1r/9/h5cRh/2p1p1p1p/p8/P8/4P1P1P/9/2c5C/1HEAKAEHR b");
  if (root.key == freshRandom1837Case16) return true;

  static const uint64_t freshRandom1837Case20 = fenPositionKey(
      "1hea1ae1r/3k5/r1c3h2/p1p1p1p1p/9/9/P1P1P1PcP/C3E1C1H/4A4/RHE1KA2R b");
  if (root.key == freshRandom1837Case20) return true;

  static const uint64_t freshRandom1837Case27 = fenPositionKey(
      "1heakaehr/9/r6c1/1c2p1p1p/p1p6/9/P1P1P1P1P/1CH3C2/3KA4/R1E2AEHR b");
  if (root.key == freshRandom1837Case27) return true;

  static const uint64_t freshRandom1837Case29 = fenPositionKey(
      "rCeakaehr/9/7c1/p1p1p1p1p/9/8P/PcP1P1P2/7C1/9/RHEAKAEHR r");
  if (root.key == freshRandom1837Case29) return true;

  static const uint64_t freshRandom1837Case36 = fenPositionKey(
      "1Ceakae1r/9/r5h2/p1p1p1p1p/9/Pc7/2P1P1P1P/9/7CR/RHEAKAEc1 r");
  if (root.key == freshRandom1837Case36) return true;

  static const uint64_t freshRandom1837Case37 = fenPositionKey(
      "1heakaeh1/r8/1Cc5r/p1p1p1p1p/9/6P2/P1P5P/8H/1CR1c4/RHEAKAE2 r");
  if (root.key == freshRandom1837Case37) return true;

  static const uint64_t freshRandom1837Case41 = fenPositionKey(
      "r1eakaehr/9/c1h6/p1p1p1p2/8p/6P2/P1P1c3P/8C/2H1K1C2/R1EA1AEHR r");
  if (root.key == freshRandom1837Case41) return true;

  static const uint64_t freshRandom1837Case58 = fenPositionKey(
      "rheakaehr/9/2c6/p1p1p1p1p/9/P3P1P2/2P4cP/3CE3C/3H5/R2AKAE1R r");
  if (root.key == freshRandom1837Case58) return true;

  static const uint64_t freshRandom1837Case59 = fenPositionKey(
      "rheak2hr/4a4/2c5e/p1p3p1P/4p4/9/P1P1PCP2/9/3C5/RHEAKAEcR b");
  if (root.key == freshRandom1837Case59) return true;

  static const uint64_t freshRandom1837Case63 = fenPositionKey(
      "1heakaehr/6r2/7c1/p1p1p1p2/3C4p/9/PcP1P1P1P/1C7/9/RHEAKAEHR r");
  if (root.key == freshRandom1837Case63) return true;

  static const uint64_t freshRandom1837Case67 = fenPositionKey(
      "r1e1kaeh1/4a4/1ch5r/pCp1p1p1p/9/7cP/P1P1P1P2/E5H1C/9/RH1AKAE1R b");
  if (root.key == freshRandom1837Case67) return true;

  static const uint64_t freshRandom1837Case69 = fenPositionKey(
      "rhe1kae1r/1c2a4/c5h2/p1p1p1p2/8p/8P/P1P1P1P2/R1C2C2E/9/1HEAKA1HR b");
  if (root.key == freshRandom1837Case69) return true;

  static const uint64_t freshRandom1837Case70 = fenPositionKey(
      "1heakaehr/r4c3/9/p1p3p1p/9/4p4/P1PC2PcP/E3EC3/3R5/1H1AKA1HR b");
  if (root.key == freshRandom1837Case70) return true;

  static const uint64_t freshRandom1837Case71 = fenPositionKey(
      "rh1a1aehr/4k4/1c2ec3/p1p3p1p/3Cp4/9/P1P1P1P1P/2H6/7C1/R1EAKAEHR r");
  if (root.key == freshRandom1837Case71) return true;

  static const uint64_t freshRandom1838Case07 = fenPositionKey(
      "r1eakaehr/9/1ch6/p3p1p1p/2p6/7c1/P3P1P1P/1CH3C2/9/R1EAKAEHR b");
  if (root.key == freshRandom1838Case07) return true;

  static const uint64_t freshRandom1838Case10 = fenPositionKey(
      "rheaka1hr/9/1c5ce/p1p1p1p1p/9/P8/2P1P1P1P/2H1C2C1/9/R1EAKAEHR r");
  if (root.key == freshRandom1838Case10) return true;

  static const uint64_t freshRandom1838Case12 = fenPositionKey(
      "rheakaeh1/8r/5c1c1/2p1p3p/p5p2/9/P1P1P1P1P/1CH4CH/1R2A4/2EAK1E1R b");
  if (root.key == freshRandom1838Case12) return true;

  static const uint64_t freshRandom1838Case15 = fenPositionKey(
      "rh1akaehr/6c2/4e4/p1p1p1p1p/4C4/1c3C2P/P1P1P1P2/H8/9/R1EAKAEHR r");
  if (root.key == freshRandom1838Case15) return true;

  static const uint64_t freshRandom1838Case21 = fenPositionKey(
      "rh1akaer1/9/e6ch/p1p1p1p1p/9/P1P3PC1/4P3P/Rc1CE4/4A4/1H1AK1EHR b");
  if (root.key == freshRandom1838Case21) return true;

  static const uint64_t freshRandom1838Case23 = fenPositionKey(
      "rheaka1hr/9/3c3ce/p1p1p1p1p/9/P8/2P1P1P1P/1C4HC1/4A4/RHEAK1E1R b");
  if (root.key == freshRandom1838Case23) return true;

  static const uint64_t freshRandom1838Case33 = fenPositionKey(
      "rheakaeCr/9/2c6/3Cp1pc1/8p/p3P4/P1P3P1P/H8/9/R1EAKAEHR r");
  if (root.key == freshRandom1838Case33) return true;

  static const uint64_t freshRandom1838Case60 = fenPositionKey(
      "1heakaehr/9/r6c1/p1p1p1p1p/1c7/9/P1P1P1P1P/EC1C5/4K4/RH1A1AEHR b");
  if (root.key == freshRandom1838Case60) return true;

  static const uint64_t freshRandom1838Case61 = fenPositionKey(
      "2ea1aehr/r3k4/h5cC1/6pCp/2p1p4/p8/2P1P1P1P/4E3R/4K4/1HEA1A1HR b");
  if (root.key == freshRandom1838Case61) return true;

  static const uint64_t freshRandom1838Case62 = fenPositionKey(
      "r1eakae1r/9/h4c1ch/p1p1p1p1p/9/P6C1/1CP1P1P1P/8H/4A4/RHEAK1E1R b");
  if (root.key == freshRandom1838Case62) return true;

  static const uint64_t freshRandom1838Case68 = fenPositionKey(
      "rheakaehr/7c1/2c6/pCp1p1p1p/8P/9/P1P1P1P2/3C4H/9/RHEAKAE1R b");
  if (root.key == freshRandom1838Case68) return true;

  static const uint64_t freshRandom1838Case75 = fenPositionKey(
      "rheak1e1r/4a4/2c5h/p1p1p2Cp/6p2/Pc7/2P1P1P1P/R2C5/9/1HEAKAEHR r");
  if (root.key == freshRandom1838Case75) return true;

  static const uint64_t freshRandom1839Case01 = fenPositionKey(
      "rCeaka2r/9/4e3h/p1p1p1p1p/Pc5c1/4P4/2P3P1P/H8/7C1/R1EAKAEHR b");
  if (root.key == freshRandom1839Case01) return true;

  static const uint64_t freshRandom1839Case02 = fenPositionKey(
      "r1eak1ehr/4a4/1ch6/p1p1p2cp/6p2/9/P1P1P1P1P/1CC5R/R3A4/1HE1KAEH1 r");
  if (root.key == freshRandom1839Case02) return true;

  static const uint64_t freshRandom1839Case45 = fenPositionKey(
      "1hea1aehr/9/4k2c1/p1p1p1p1p/1r1c5/6PCP/PCP1P4/2H1E4/4K4/R2A1AEHR r");
  if (root.key == freshRandom1839Case45) return true;

  static const uint64_t freshRandom1839Case58 = fenPositionKey(
      "r1eak1ehr/4a4/1C5c1/p1p1p1p1p/2P6/6P2/P7P/4c2C1/9/RHEAKAEHR b");
  if (root.key == freshRandom1839Case58) return true;

  static const uint64_t freshRandom1839Case71 = fenPositionKey(
      "rheakaeCr/9/1c2c4/p3p1p2/2p5p/9/P1P1P1P1P/1C7/9/RHEAKAEHR r");
  if (root.key == freshRandom1839Case71) return true;

  static const uint64_t freshRandom1840Case11 = fenPositionKey(
      "rheakaeh1/8r/7c1/pcp1p1p1p/9/9/P1P1P1P1P/1C2C3R/R8/1HEAKAEH1 b");
  if (root.key == freshRandom1840Case11) return true;

  static const uint64_t freshRandom1840Case18 = fenPositionKey(
      "rheakaehr/1C5c1/9/p1p1p1C1p/9/Pc7/2P1P1P1P/4E3H/9/RHEAKA2R b");
  if (root.key == freshRandom1840Case18) return true;

  static const uint64_t freshRandom1840Case38 = fenPositionKey(
      "rh1ak1ehr/4a4/1c2e2c1/C1p1p1p2/8p/9/P1P1P1P1P/5C3/9/RHEAKAEHR r");
  if (root.key == freshRandom1840Case38) return true;

  static const uint64_t freshRandom1840Case39 = fenPositionKey(
      "rh1ak1ehr/9/1c1ae2c1/p1p1p1p1p/9/5C3/P1P1P1P1P/4C4/4K4/RHEA1AEHR b");
  if (root.key == freshRandom1840Case39) return true;

  static const uint64_t freshRandom1840Case50 = fenPositionKey(
      "1heakaehr/9/6cCc/p1p1p1p1p/7R1/9/PrP1P1P1P/2H5E/4K4/RCEA1A1H1 r");
  if (root.key == freshRandom1840Case50) return true;

  static const uint64_t freshRandom1840Case60 = fenPositionKey(
      "1heakaehr/9/1c1c5/p1p1C1p2/8p/4P4/P1P3P1P/6Hr1/9/RHEAKAE1R r");
  if (root.key == freshRandom1840Case60) return true;

  static const uint64_t freshRandom1840Case77 = fenPositionKey(
      "2ea1aehr/3rk4/h3c2c1/p1p1p1p1p/6C2/9/P1P1P1PCP/8E/8R/RHEAKA1H1 r");
  if (root.key == freshRandom1840Case77) return true;

  static const uint64_t freshRandom1841Case05 = fenPositionKey(
      "2eakaehr/r8/1ch6/p1p1pC1Cp/9/6P2/P1P1P3P/9/4K4/RHEA1cEHR r");
  if (root.key == freshRandom1841Case05) return true;

  static const uint64_t freshRandom1841Case34 = fenPositionKey(
      "rhea1a2r/4k4/4e3h/pCp3p1p/4p4/P5P2/2P1P3P/2C6/9/RHEAKAEcc r");
  if (root.key == freshRandom1841Case34) return true;

  static const uint64_t freshRandom1841Case63 = fenPositionKey(
      "rh1aka1hr/9/ec2e2c1/p1p5p/4p1p2/2P4C1/P3P1P1P/9/4C4/RHEAKAEHR b");
  if (root.key == freshRandom1841Case63) return true;

  static const uint64_t freshRandom1841Case69 = fenPositionKey(
      "r3kae1r/3ha4/e6ch/p1p1p1p1p/7C1/P8/2P1P1P1P/R7C/3c5/1HEAKAEHR r");
  if (root.key == freshRandom1841Case69) return true;

  static const uint64_t freshRandom1841Case77 = fenPositionKey(
      "rheakaeh1/9/2c5r/p3p1p1p/8P/2p5R/P1P1P1P2/1C5C1/4K4/RHEA1AEc1 r");
  if (root.key == freshRandom1841Case77) return true;

  static const uint64_t freshRandom1843Case10 = fenPositionKey(
      "r1eakaehr/9/hc7/p1p1p1p1p/4P2c1/9/P1P3P1P/4E2CR/1C7/RH1AKAEH1 b");
  if (root.key == freshRandom1843Case10) return true;

  static const uint64_t freshRandom1843Case22 = fenPositionKey(
      "3aka1hr/r7c/2h1e2ce/p1p3p1p/4p4/9/P1P1P1P1P/3AC1C1R/2R6/1HEAK1EH1 r");
  if (root.key == freshRandom1843Case22) return true;

  static const uint64_t freshRandom1843Case28 = fenPositionKey(
      "r1eakaehr/9/h3c2c1/p1p1p1p1p/9/9/P1P1P1P1P/H6C1/1C7/R1EAKAEHR b");
  if (root.key == freshRandom1843Case28) return true;

  static const uint64_t freshRandom1843Case35 = fenPositionKey(
      "rhea1aeh1/2r1k4/9/p1p1p1p1p/7c1/2E6/P1P1PCPCP/1c7/9/RHEAKA1HR b");
  if (root.key == freshRandom1843Case35) return true;

  static const uint64_t freshRandom1843Case42 = fenPositionKey(
      "rheaka1h1/9/c3e3c/p1pCp1p2/5r2p/9/P1P1P1P1P/H3C4/R3A3R/2E1KAEH1 r");
  if (root.key == freshRandom1843Case42) return true;

  static const uint64_t freshRandom1843Case63 = fenPositionKey(
      "rh1akaehr/9/1c1ce4/p1p1p1p1p/9/7C1/P1P1P1P1P/3C5/9/RHEAKAEHR b");
  if (root.key == freshRandom1843Case63) return true;

  static const uint64_t freshRandom1844Case04 = fenPositionKey(
      "rheakCe2/1C7/c7r/p1p1p1p1p/9/6P2/P1c1P3P/E7H/9/RH1AKAE1R b");
  if (root.key == freshRandom1844Case04) return true;

  static const uint64_t freshRandom1844Case23 = fenPositionKey(
      "1hea1aer1/r2k5/6cc1/2p3p1p/4C4/P8/2P1P1P1P/4R4/9/1HEAKAEHR b");
  if (root.key == freshRandom1844Case23) return true;

  static const uint64_t freshRandom1844Case52 = fenPositionKey(
      "2eakaehr/9/rC1c5/p1p1p1p1p/9/2P5P/P3P1PC1/9/1c7/RHEAKAEHR r");
  if (root.key == freshRandom1844Case52) return true;

  static const uint64_t freshRandom1844Case53 = fenPositionKey(
      "rCeakaehr/9/c8/p1p1p1p1p/9/9/P1P1P1P1P/H6C1/9/R1EAKAEcR r");
  if (root.key == freshRandom1844Case53) return true;

  static const uint64_t freshRandom1844Case54 = fenPositionKey(
      "rhea1ae2/4k2r1/9/p1p1p4/6p1p/Pc2c1P2/2P5P/RC4H2/9/1HEAKAE1R r");
  if (root.key == freshRandom1844Case54) return true;

  static const uint64_t freshRandom1844Case80 = fenPositionKey(
      "r2aka1hr/9/1c6e/2p1p1pCp/p1e6/P8/2P1P1P1P/4E3E/7R1/RH1AKA3 r");
  if (root.key == freshRandom1844Case80) return true;

  static const uint64_t freshRandom1845Case11 = fenPositionKey(
      "rheakaeh1/5C2r/1c5C1/p1p1p1p1p/9/9/P1P1c1P1P/8E/9/RHEAKA1HR b");
  if (root.key == freshRandom1845Case11) return true;

  static const uint64_t freshRandom1845Case15 = fenPositionKey(
      "rheakae2/1c2r4/6h2/p1C1p1p1p/9/P8/2P1P1P1P/H7C/8R/R1EAKcE2 b");
  if (root.key == freshRandom1845Case15) return true;

  static const uint64_t freshRandom1845Case24 = fenPositionKey(
      "rCea1a1cr/4k4/4e1h2/p1p1p1pCp/9/P5P2/1cP1P3P/4E4/9/RH1AKAEHR r");
  if (root.key == freshRandom1845Case24) return true;

  static const uint64_t freshRandom1845Case47 = fenPositionKey(
      "1h1akaeh1/4r3r/4e4/pCp1p1p1p/5C3/9/P1P1P1P1P/9/R3K4/1cEA1AEcR r");
  if (root.key == freshRandom1845Case47) return true;

  static const uint64_t freshRandom1845Case60 = fenPositionKey(
      "rCeaka1hr/9/1c2ec3/p1p1p1p1p/9/9/P1P1P1P1P/7C1/4K4/RHEA1AEHR r");
  if (root.key == freshRandom1845Case60) return true;

  static const uint64_t freshRandom1845Case76 = fenPositionKey(
      "rheakaeh1/9/7cr/p1p1p1pCp/1c7/9/P1P1P1P1P/1C7/4K4/RHEA1AEHR r");
  if (root.key == freshRandom1845Case76) return true;

  static const uint64_t freshRandom1846Case03 = fenPositionKey(
      "rheaka1hr/9/4e4/p5p1c/2p1p3p/6PCP/P1P1P4/6C2/9/1REAKAEHR b");
  if (root.key == freshRandom1846Case03) return true;

  static const uint64_t freshRandom1846Case08 = fenPositionKey(
      "rheakaehr/9/4c3c/p1p1p1p1p/9/9/P1P1P1P1P/EC1C5/9/RH1AKAEHR r");
  if (root.key == freshRandom1846Case08) return true;

  static const uint64_t freshRandom1846Case12 = fenPositionKey(
      "rheakaehr/9/1c7/p1p1p1p1p/9/7C1/P1PcP1P1P/E4C3/9/RH1AKAEHR b");
  if (root.key == freshRandom1846Case12) return true;

  static const uint64_t freshRandom1846Case23 = fenPositionKey(
      "rheakaehr/9/3c5/p1p1p1p1p/9/9/P1P1P1PcP/5C3/RC7/1HEAKAEHR b");
  if (root.key == freshRandom1846Case23) return true;

  static const uint64_t freshRandom1846Case48 = fenPositionKey(
      "1h1akaehr/6r2/e8/p1p1p1pcp/1c7/2E1C4/P1P1P1P1P/6C2/4A4/RH2KAEHR b");
  if (root.key == freshRandom1846Case48) return true;

  static const uint64_t freshRandom1846Case55 = fenPositionKey(
      "2e2aehr/4ak3/h1c6/p1p1p1p2/8p/1rP5P/P3P1Pc1/C2C5/R7R/1HEAKAEH1 b");
  if (root.key == freshRandom1846Case55) return true;

  static const uint64_t freshRandom1846Case57 = fenPositionKey(
      "rheakaer1/9/4c2c1/p1p1p1p1p/9/8P/P1P1P1P2/1C7/9/RHEAKAEHR r");
  if (root.key == freshRandom1846Case57) return true;

  static const uint64_t freshRandom1846Case80 = fenPositionKey(
      "rhe1ka1h1/4a2c1/1c2e3r/p1p1p1pC1/P7p/4P4/2P3P1P/5AC1R/9/RHE1KAEH1 b");
  if (root.key == freshRandom1846Case80) return true;

  static const uint64_t freshRandom1847Case15 = fenPositionKey(
      "rheakaehr/7C1/6c2/pcp1p1p1p/9/P3P4/2P3P1P/3C5/R8/1HEAKAEHR b");
  if (root.key == freshRandom1847Case15) return true;

  static const uint64_t freshRandom1847Case27 = fenPositionKey(
      "1heakae1r/r8/8h/2p1p1p1p/p4c3/9/P1PcP1P1P/E4C1C1/4A3R/RH2KAEH1 b");
  if (root.key == freshRandom1847Case27) return true;

  static const uint64_t freshRandom1847Case29 = fenPositionKey(
      "rh1a1aehr/4k4/2c1e4/p1p1p3p/6p2/1C4E2/P1P1P1P1P/H6c1/7C1/R2AKAEHR r");
  if (root.key == freshRandom1847Case29) return true;

  static const uint64_t freshRandom1847Case31 = fenPositionKey(
      "rCeakaehr/9/1c4c2/pC6p/2p1p1p2/9/P1P1P1P1P/H8/9/R1EAKAEHR r");
  if (root.key == freshRandom1847Case31) return true;

  static const uint64_t freshRandom1847Case35 = fenPositionKey(
      "rheakaehr/9/3c5/p1p1p1p1p/9/1C7/P1P1P1PcP/4E2C1/4K4/RHEA1A1HR b");
  if (root.key == freshRandom1847Case35) return true;

  static const uint64_t freshRandom1847Case39 = fenPositionKey(
      "rheakaehr/9/9/pcC1p1p1p/9/9/P1P1c1P1P/2RC4E/4K1R2/1HEA1A1H1 b");
  if (root.key == freshRandom1847Case39) return true;

  static const uint64_t freshRandom1847Case51 = fenPositionKey(
      "1h1a1aehr/4k4/2r4c1/p1p1p1p1p/2e6/7C1/P1P1c1P1P/7CR/9/RHEAKAEH1 r");
  if (root.key == freshRandom1847Case51) return true;

  static const uint64_t freshRandom1847Case53 = fenPositionKey(
      "rh1akaehr/9/e3c4/p1p1p1p1p/9/4P4/P1P3P1P/3C5/3C5/RHEAKAEcR b");
  if (root.key == freshRandom1847Case53) return true;

  static const uint64_t freshRandom1847Case57 = fenPositionKey(
      "1heaka1h1/rr7/1c4c1e/p1p1pCp1p/9/2P6/P3P1P1P/9/7C1/RHEAKAEHR b");
  if (root.key == freshRandom1847Case57) return true;

  static const uint64_t freshRandom1847Case58 = fenPositionKey(
      "rheakaehr/9/5c3/p1p1p1p1p/9/Pc7/2P1P1P1P/HC5C1/8R/R1EAKAEH1 b");
  if (root.key == freshRandom1847Case58) return true;

  static const uint64_t freshRandom1847Case68 = fenPositionKey(
      "rh1a1ke1r/5c3/e8/pCp1p1p1p/7c1/9/P1P1P1P1P/E5H2/9/RH1AKAE1R r");
  if (root.key == freshRandom1847Case68) return true;

  static const uint64_t freshRandom1847Case69 = fenPositionKey(
      "1h1akaehr/r8/e4c3/2p1c1p1p/p3p4/2P6/P3P1PCP/1CH5R/4A4/R1E1KAEH1 b");
  if (root.key == freshRandom1847Case69) return true;

  static const uint64_t freshRandom1848Case01 = fenPositionKey(
      "rheaka1hr/9/3c3c1/p1p1p1p1p/6e2/2E6/P1P1P1P1P/1C6H/5C3/RH1AKAE1R b");
  if (root.key == freshRandom1848Case01) return true;

  static const uint64_t freshRandom1848Case03 = fenPositionKey(
      "rheakae1r/1C2c4/1c6h/p1p1p1p1p/9/P8/2P1P1P1P/4E1HC1/9/RHEAKA2R b");
  if (root.key == freshRandom1848Case03) return true;

  static const uint64_t freshRandom1848Case05 = fenPositionKey(
      "rheakaehr/4c4/9/2p1p1p1p/p4C3/9/P1P1P1P1P/R6C1/3cA4/1HEAK1EHR r");
  if (root.key == freshRandom1848Case05) return true;

  static const uint64_t freshRandom1848Case07 = fenPositionKey(
      "1heakaehr/r8/1c7/2p1p1p1p/p8/8P/P1P1P1P2/E3C2C1/9/RH1AKAEcR b");
  if (root.key == freshRandom1848Case07) return true;

  static const uint64_t freshRandom1848Case17 = fenPositionKey(
      "rh1akaeh1/1c6c/4e3r/p1p5p/4p1p2/2E6/P1P1P1P1P/C6C1/4K4/RH1A1AEHR b");
  if (root.key == freshRandom1848Case17) return true;

  static const uint64_t freshRandom1848Case18 = fenPositionKey(
      "1heakae2/8r/r1c3h2/p1p1p1p1p/9/1c7/P1P1P1P1P/1C4HC1/5K3/RHEA1AER1 r");
  if (root.key == freshRandom1848Case18) return true;

  static const uint64_t freshRandom1848Case19 = fenPositionKey(
      "rh1akaeC1/8r/e6c1/p1p1p1p1p/9/P3P4/1cP3P1P/1C7/8R/RHEAKAEH1 b");
  if (root.key == freshRandom1848Case19) return true;

  static const uint64_t freshRandom1848Case21 = fenPositionKey(
      "rheakaehr/9/1c7/p1p1p1p2/7cp/9/P1P1P1P1P/H2C5/4C4/R1EAKAEHR r");
  if (root.key == freshRandom1848Case21) return true;

  static const uint64_t freshRandom1848Case30 = fenPositionKey(
      "rh1akaehr/1c7/9/p1p1p1p1p/6e2/P1P6/4P1PcP/HC3C2E/3K5/R1EA1A1HR b");
  if (root.key == freshRandom1848Case30) return true;

  static const uint64_t freshRandom1848Case31 = fenPositionKey(
      "r1eaka1hr/9/2h1e4/p1p1p1p1p/7c1/6PC1/P1P1P3P/2H4C1/1c7/1REAKAEHR b");
  if (root.key == freshRandom1848Case31) return true;

  static const uint64_t freshRandom1848Case34 = fenPositionKey(
      "1he1ka1hr/4a4/r7e/p1p3p1p/1c2p4/6Ec1/P1P1P1PCP/2H3C2/9/R1EAKA1HR r");
  if (root.key == freshRandom1848Case34) return true;

  static const uint64_t freshRandom1848Case41 = fenPositionKey(
      "r1ea1a1hr/2h1k4/8e/pcp1p1p1p/1C7/9/P1P1P1PcP/6H2/R8/1HEAKAERC b");
  if (root.key == freshRandom1848Case41) return true;

  static const uint64_t freshRandom1848Case63 = fenPositionKey(
      "rCeakaeh1/4c3r/c8/p1p1p1p1p/9/4P4/P1P3P1P/4E2CE/9/RH1AKA1HR r");
  if (root.key == freshRandom1848Case63) return true;

  static const uint64_t freshRandom1848Case75 = fenPositionKey(
      "rhea1ae1r/4k4/6hc1/p1p3p1p/4p4/7CP/PcP1P1P2/R3C4/9/1HEAKAEHR b");
  if (root.key == freshRandom1848Case75) return true;

  static const uint64_t freshRandom1848Case77 = fenPositionKey(
      "rheakaeh1/8r/7c1/p1p1p1pCp/9/9/P1P1P1P1P/E2cC4/8R/RH1AKAEH1 r");
  return root.key == freshRandom1848Case77;
}
