...
- A local C++ UCI engine (`native/xiangqi_native.cpp`) with legal Xiangqi move generation, alpha-beta/PVS search, quiescence, Zobrist-keyed transposition reuse with hash-move ordering, MultiPV telemetry, and a `local-cpp` preset for native search without downloading another engine.
