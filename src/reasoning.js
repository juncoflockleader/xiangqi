...
function searchTechniqueReasons(stats = {}) {
  ...
  if ((stats.qttMoveHits ?? 0) > 0) {
    orderingParts.push(formatCount(stats.qttMoveHits, "quiescence hash-move ordering hint"));
  }
  ...
  if ((stats.checkHistoryHits ?? 0) > 0) {
    orderingParts.push(formatCount(stats.checkHistoryHits, "check-history hit"));
  }
  if ((stats.killerHits ?? 0) > 0) {
    orderingParts.push(formatCount(stats.killerHits, "killer-move hit"));
  }
  ...
  const continuationReductionAdjustments = (stats.continuationReductionBoosts ?? 0) + (stats.continuationReductionMaluses ?? 0);
  if (continuationReductionAdjustments > 0) {
    orderingParts.push(formatCount(continuationReductionAdjustments, "continuation-history LMR adjustment"));
  }
...
function searchSelectivityConfidenceFactor(stats = {}) {
  ...
  if ((stats.continuationReductionBoosts ?? 0) > 0 || (stats.continuationReductionMaluses ?? 0) > 0) {
