...
test("local C++ engine builds and searches through the native UCI backend", async (t) => {
  ...
  try {
    ...
    assert.ok(result.raw.some((line) => /\bqtt\b/.test(line) && /\bqttstores\b/.test(line) && /\beval\b/.test(line) && /\bevalskip\b/.test(line) && /\bevalguard\b/.test(line) && /\brep\b/.test(line)));
