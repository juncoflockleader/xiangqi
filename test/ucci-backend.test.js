...
test("native backend preserves search telemetry from info lines", async () => {
  ...
  try {
    ...
    assert.equal(result.stats.followupHistoryStores, 10);
    assert.equal(result.stats.followupReductionBoosts, 4);
    assert.equal(result.stats.followupReductionMaluses, 2);
    ...
    assert.equal(result.stats.pawnThreatOrderHits, 3);
    ...
    assert.equal(result.iterations[0].stats.qttPrefetches, 22);
    assert.equal(result.iterations[0].stats.evalCacheHits, 19);
    assert.equal(result.iterations[0].stats.evalCachePrefetches, 44);
    assert.equal(result.iterations[0].stats.checkedEvalSkips, 31);
    assert.equal(result.iterations[0].stats.staticEvalTrendClears, 4);
    assert.equal(result.iterations[0].stats.ttMoveHits, 6);
    assert.equal(result.iterations[0].stats.ttPrefetches, 33);
    ...
    assert.equal(result.iterations[0].stats.pawnThreatOrderHits, 3);
    ...
    assert.equal(result.iterations[0].stats.followupHistoryStores, 10);
    assert.equal(result.iterations[0].stats.followupReductionBoosts, 4);
    assert.equal(result.iterations[0].stats.followupReductionMaluses, 2);
    ...
    assert.ok(result.explanation.reasons.some((reason) => reason.includes("quiescence-table prefetch requests")));
    assert.ok(result.explanation.reasons.some((reason) => reason.includes("evaluation-cache hits")));
    assert.ok(result.explanation.reasons.some((reason) => reason.includes("evaluation-cache prefetch requests")));
    assert.ok(result.explanation.reasons.some((reason) => reason.includes("transposition-table prefetch requests")));
    ...
    assert.ok(result.explanation.reasons.some((reason) => reason.includes("static eval-trend reset")));
    ...
    assert.ok(result.explanation.reasons.some((reason) => reason.includes("follow-up reduction boost")));
